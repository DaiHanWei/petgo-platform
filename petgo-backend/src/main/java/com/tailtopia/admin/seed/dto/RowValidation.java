 ...
 *
 * @param errors    硬错误（文案码 + 实参，bug 20260924-565：展示端按后台语言渲染）。非空 ⇒ 该行**不发**，留在草稿态可改后重提。
 *                  🛡 <b>不阻塞整批</b>（AC2）。
 * @param duplicate 去重命中。🔴 <b>这是提示不是错误</b>（AC4）——
 *                  由运营决定是否仍要发布。原先是**静默跳过**，界面只显示一个跳过条数，
 *                  运营根本不知道哪一条被吞了。
 */
public record RowValidation(SeedBatchRow row, List<RowError> errors, boolean duplicate) {
