  ...
  group('AC8 命名规范（全部事件与属性 snake_case）', () {
    ...
    test('lib/ 里每个非遗留事件都符合「模块前缀 + 对象 + 动作」', () {
      ...
      const allowedPrefixes = <String>[
        ...
        'post_',
        // 全屏图片查看器（V1.3.0 批次 A · FR-115 / Story 3.3）。模块是「灯箱」这件事本身 ——
        // 它是**跨模块的共享组件**（本批次挂详情页，批次 B1 的场所照片直接接入同一个），
        // 挂进 post_ 会让场所那边的打开数被算成内容帖的。
        // ⚠️ 新模块入表，不是为遗留事件放宽规则。事件名由 PRD §4 定死，
        //    AD-A23 明令实现不得改名 —— 所以扩表、不改名。
        'lightbox_',
        // 宠物年龄卡（V1.3.0 批次 A · FR-65 / Story 5.2）。模块是「年龄卡」这件事 ——
        // 它挂在成长档案的聚合页下，但事件描述的对象是这张卡本身，`age_card_*` 一眼可读。
        // ⚠️ 新模块入表。事件名由 PRD §4 定死，AD-A23 明令实现不得改名 —— 扩表、不改名。
        'age_card_',
        ...
        'login_',
        // @ 提及（V1.3.0 batch-b1 FR-119 / Story 3.5 的 E-13）。模块是「@ 这件事」——
        // 它横跨发布页输入框、评论输入框、Feed 卡片正文、详情页正文、评论正文**五处界面**，
        // 挂到任何单页前缀下都会误导（`social_mention_*` 会让人以为只有首页有）。
        ...
        //    会被当成同一漏斗的上下游。KTP 是产品对这张卡的叫法，一眼可读。
        // ⚠️ 这是**新模块**入表，不是为遗留事件放宽规则。
        'ktp_',
      ...
      const allowedSuffixes = <String>[
        ...
        // 插入完成（V1.3.0 batch-b1 Story 3.5 的 E-13 `mention_inserted`）：
        // `_inserted` = **那个 @ 真的进了文本**，与「打开了选择器」「点了某一行」分得开 ——
        // 被 5 人上限或字数上限拦住的那次压根不报（漏斗的分母是选择器曝光，不是点击）。
        // 🔴 刻意不用 `_selected`：选中但没插进去也叫 selected，而这里要数的是**结果**。
        '_inserted',
        '_responded',
        ...
        '_blocked',
        // 一个临时层被关掉了 —— 两种用法共用 `_dismissed`：
        // ① 面板被关掉而**没完成**（V1.3.0 shop-v2 Story 1-4 的 `toko_payment_sheet_dismissed`）：
        //    用户主动关掉了一个还没走完的流程。🔴 刻意不用 `_closed`：关闭是中性的，而这条事件的
        //    全部意义在于「他放弃了」—— 它是支付漏斗里「出码之后自己走掉」那一格，服务端**没有任何对应事件**
        //    （关面板不产生服务端状态变化），只有客户端看得见。🔴 也刻意不用 `_tapped`：那会被当成
        //    「取消按钮的全部点击数」拿去做分母，而这条只统计「未完成就关」，付成功后面板自动关不算。
        // ② 查看器被关闭（V1.3.0 批次 A · Story 3.3）：看图没有"完成"一说；也不用 `_tapped`，
        //    因为关闭有四种方式（✕ / 单击 / 下滑 / 系统返回），具体哪种由属性 dismiss_gesture 承载。
        '_dismissed',
        // 卡片被分享出去（V1.3.0 批次 A · Story 5.2）：`_shared` = **用户把图递给了系统面板**。
        ...
        '_dwell',
        // 付费失败（2026-09-25 KTP 付费漏斗 `ktp_unlock_failed`，名字按运营需求）：`_failed` = 这次付费
        // **有了明确的失败结局**，与 `_succeeded` 成对，原因由属性 failure_reason 承载。
