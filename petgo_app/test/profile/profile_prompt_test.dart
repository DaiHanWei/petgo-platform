...
void main() {
  ...
  group('FR-0H 提示条状态机（纯函数）', () {
    test('A 用户、未创建、第 1/2/3 次重启 → 显示；第 4 次（>3）→ 不显示', () {
      expect(show('A', s(restart: 1)), isTrue);
      expect(show('A', s(restart: 2)), isTrue);
      expect(show('A', s(restart: 3)), isTrue);
      expect(show('A', s(restart: 4)), isFalse);
    });
    ...
    test('当次关闭（count<3）→ 本 session 不显示、计数不增、下次重启仍显示', () {
      ...
      expect(show('A', dismissed), isFalse); // 本 session 不显示
      ...
      expect(show('A', nextLaunch), isTrue);
    });

    test('第 3 次重启关闭 → 永久关闭，之后任何重启不显示', () {
      final dismissed = onDismiss(s(restart: 3));
      expect(dismissed.dismissedPermanently, isTrue);
      expect(show('A', dismissed), isFalse);
      // 后续重启即使计数仍 ≤3 也不显示
      expect(show('A', s(restart: 2, permanent: true)), isFalse);
    });

    test('任意时刻 petProfileCompleted → 不显示', () {
      expect(show('A', s(restart: 1, completed: true)), isFalse);
      expect(show('A', onProfileCompleted(s(restart: 1))), isFalse);
    });

    test('hasPetProfile（Epic 2 回填 true）→ 即使计数未满也不显示', () {
      expect(show('A', s(restart: 1), hasPetProfile: true), isFalse);
    });

    test('B/C 用户 → 任何情况都不显示（计数不激活）', () {
      expect(show('B', s(restart: 1)), isFalse);
      expect(show('C', s(restart: 1)), isFalse);
      expect(show('B', s(restart: 3)), isFalse);
