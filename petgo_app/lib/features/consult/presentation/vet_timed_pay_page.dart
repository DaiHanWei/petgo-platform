...
class _VetTimedPayPageState extends ConsumerState<VetTimedPayPage> {
  ...
  Future<void> _tick() async {
    try {
      final s = await ref
          ...
          .requestStatus(widget.requestToken);
      if (!mounted) return;
      if (s.payDeadlineAt != null) {
        _deadline = s.payDeadlineAt; // 服务端权威纠偏（含 resume 顺延）
      }
      setState(_recompute);
  ...
  Future<void> _topUp() async {
    try {
      await ref
          .read(consultRepositoryProvider)
          .pauseRequest(widget.requestToken);
    } catch (_) {
      /* 暂停失败也继续跳充值（返回后 resume 会纠偏） */
    }
    if (!mounted) return;
    ...
    try {
      await ref
          .read(consultRepositoryProvider)
          .resumeRequest(widget.requestToken);
    } catch (_) {
      /* resume 失败 → 下个 tick 轮询纠偏（可能已超时 404） */
    }
    // 刷新余额 + 重拉状态续倒计时。
  ...
  Future<void> _cancelPay() async {
    ...
    try {
      await ref
          .read(consultRepositoryProvider)
          .cancelRequest(widget.requestToken);
    } catch (_) {
  ...
  Widget build(BuildContext context) {
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        body: SafeArea(
          child: _awaitingCash ? _buildAwaitingCash(l10n) : _buildPay(l10n),
        ),
      ),
    );
  }
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                ...
                _channelTile(
                  'PAWCOIN',
                  l10n.triageUnlockMethodPawcoin,
                  l10n.triageUnlockPawcoinBalance(formatKoin(balance)),
                  Icons.account_balance_wallet_outlined,
                ),
                const SizedBox(height: 10),
                _channelTile(
                  'QRIS',
                  l10n.triageUnlockMethodQris,
                  'QRIS',
        ...
        child: Row(
          children: [
            Icon(
              icon,
              size: 22,
              color: selected ? AppColors.mint : AppColors.ink2,
            ),
