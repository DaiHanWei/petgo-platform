...
///
/// ## 🛡 三段顺序不可调换
/// ① 取图片实际比例 → ② 收敛到 `0.75~1.78` **闭区间** → ③ 施加高度护栏。
///
...
/// 容差区间上界。**闭区间含本值** —— 16:9 恰为 1.777…，含在区间内。
///
/// 🔁 **2026-09-11 由 1.34 放宽到 1.78**（原值 ≈4:3 横拍）。起因：KTP 身份证卡
/// 导出图是 1988×1200（≈1.66），落在 1.34 之外 → 发布时被 [needsCrop] 拦去裁剪页，
/// 而裁剪页只有 1:1 / 4:5 两档，一张横卡只能被裁成方的或竖的，主体直接缺角。
/// 取 1.78 而不是刚好够用的 1.70：手机录屏、截图、运营横向素材绝大多数是 16:9，
/// 一并含进来才不会下个月再为 0.03 的差距改一次。
///
/// 🛡 放宽**上界**不影响布局安全：比例越大图片越矮，挤不到下一条内容；
/// 真正受高度护栏约束的是下界那一侧（竖图），[kFeedRatioMin] 保持不动。
const double kFeedRatioMax = 1.78;

/// 尺寸未知时的预留比例（存量内容）。
///
/// 选 1:1 的理由：因为有 clamp，最终比例必落在 `0.75~1.78`；
/// 从 1:1 出发，最坏也只是变高三成或变矮四成四 —— **跳动有硬上下界**。
...
/// 写成 `r > 0.75 && r < 1.78` 会把最常见的 3:4 竖拍排除在外、恰好裁掉最该保护的那种图。
