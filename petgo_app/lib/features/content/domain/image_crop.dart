...
/// 裁剪预设。**三档，全是方/竖**，按竖向程度排列（1:1 → 4:5 → 3:4）。
///
/// 🔁 bug 20260922-535 起加 3:4 竖版档（原先只有 1:1 / 4:5 两档）。
/// 3:4 = 0.75 恰为 [kFeedRatioMin]，闭区间含端点 → 裁出来的图展示端**不会再被二次裁**
/// （整数像素取整最多偏半个像素，展示端 clamp 后 cover 掉的不足 1px，肉眼不可见）。
///
/// 🛡 **仍然刻意不提供横图预设**：上界 2026-09-11 再放宽到 1.78 之后，连 16:9 都免裁直接过，
/// 落进裁剪框的图已经很少，再加横图档只会让裁剪页交互变重。
/// 因此真正的超宽全景（3:1、4:1）仍只能裁成这几档方/竖比例 —— 这是**已知且接受**的结果，不是缺陷。
/// 谁想"顺手补个 16:9 档"，先看这段：16:9 现在根本不会走到裁剪页。
enum CropPreset {
  square('1:1', 1.0),
  portrait('4:5', 4 / 5),

  /// 3:4 竖版（bug 20260922-535）。埋点取值自动为 `3x4`。
  tallPortrait('3:4', 3 / 4);
  ...
  /// 埋点取值（PRD 口径 `1x1` / `4x5`，bug 20260922-535 新增 `3x4`）。
  ///
...
Future<List<Uint8List>?> applyBatchCrop(
  ...
  for (final bytes in images) {
    ...
    //
    // 🔴 这个分布是**判断容差区间 0.75~1.78 取值是否合适的唯一依据**，
