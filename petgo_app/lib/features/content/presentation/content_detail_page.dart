...
class _ImageCarouselState extends State<_ImageCarousel> {
  ...
  void _openLightbox(int index) {
    ...
    // 🔴 V1.3.0 batch-b1 Story 1.6：实现搬到了公共组件 `PhotoLightbox`，**行为一字未改**
    //    （场所详情页也要用它）。别在这里加参数改行为 —— 重做灯箱是批次 A 的 FR-115。
    openPhotoLightbox(context, urls: widget.urls, initialIndex: index);
  }
