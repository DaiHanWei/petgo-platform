...
///
/// ⚠️ **本文件的两个 tile 是 V1.0.0 遗留实现，Story 3.3 起只剩「当天详情页」还在用**。
/// 五类条目的唯一渲染组件是 `widgets/timeline_item_tile.dart`（Story 2.2 建立；游客示例本与
/// **真实时间线**都已迁过去）。**Story 3.4 把当天详情也迁完后，本文件应整体删除**
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  // 副标题恒渲染（bug 20260721-341 同高度一致性）：有摘要=可点展开/收起（stag 增强），
                  // 无摘要回退「点击查看结果」提示，避免一条两行一条一行。
