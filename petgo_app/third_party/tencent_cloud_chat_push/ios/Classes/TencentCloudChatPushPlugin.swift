...
public class TencentCloudChatPushPlugin: NSObject, FlutterPlugin {
    ...
    private var _hasCalledNotificationClick = false
    private let notificationClickQueue = DispatchQueue(label: "com.timpush.notificationClick.queue")
    public var hasCalledNotificationClick: Bool {
        get { notificationClickQueue.sync { _hasCalledNotificationClick } }
        set { notificationClickQueue.sync { _hasCalledNotificationClick = newValue } }
    }
    ...
    public func tryNotifyDartOnNotificationClickEvent(_ extString: String?) {
        guard !hasCalledNotificationClick else {
            print("hasCalledNotificationClick")
            return
        }

        if registeredOnNotificationClickEvent {
            hasCalledNotificationClick = true
            toFlutterMethod("on_notification_clicked", data: extString ?? "")

            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) { [weak self] in
                self?.hasCalledNotificationClick = false
            }
        } else {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) { [weak self] in
