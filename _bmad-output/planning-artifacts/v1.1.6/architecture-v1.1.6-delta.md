...
baseline:
  ...
  - _bmad-output/planning-artifacts/v1.1.2/architecture-v1.1.2-delta.md # V1.1.2 已完成
...
companions:
  - _bmad-output/planning-artifacts/v1.1.6/.memlog-architecture-2026-08-11.md
...
- **Rule:**
  ...
  2. **高度护栏**：图片区高度上限 = 可视区高度 − 条目其余部分高度 − 露出余量。超出即 cover 居中裁切。
     ✅ **2026-08-18 小屏实机复核后定死（OA-1 已闭合）**，三个被减项的口径一并落定：
     - **可视区高度** = 滚动视口的实测高度（列表容器用 `LayoutBuilder` 量），顶部标签行与底部导航栏已被外层扣除。360×640 小屏实测 **470**。
     - **条目其余部分高度 = 142**（作者行 42 + 操作行 30 + 正文 2 行 50 + 时间行 20）。**不含条目间隔**。
     - **露出余量 = 40**（起点值 56 → 改 40），口径 = 条目间隔 25 + 下一条头像露出 15。
     🔴 改动理由是实机量出的**回归**：起点值 56 且把条目间隔重复计一遍时，小屏上图片区上限被压到 247，
     **比改版前写死的 4:3（270）还矮** —— 竖图裁得比改版前更狠，与本次改版"少裁"的初衷相反。
     改后上限 288，在最小屏上也不劣于改版前。客户端有专门的回归测试钉住这条。
...
- **Rule:**
  ...
  8. 手机号软引导维持「**全局仅一次**」，**不跟随** FR-85 的多触发点模型。
     **新时机（2026-08-20 产品口径，决策 X-21，闭合 OA-5 的唯一硬阻塞）：用户第 3 天打开 App 时提示。**
     ✅ **下列四点口径由研发拟定，产品已于 2026-08-21 逐条确认** —— 不再是待驳回的推导。
     旧的「首次问诊后 / 建档后取最早」在线上已不再触发（第二代置位了共用标记键），**不得指望复用旧门控**。
     判定口径（四点全部是可验收断言）：
     - **第 1 天 = 注册当天** ⇒ 第 3 天 = `users.created_at` + 2 个自然日；
     - 自然日按 **WIB（`Asia/Jakarta`）** 划界。⚠️ 这是本项目**第一处把时区用于判定而非展示**的地方 ——
       既有三处 `Asia/Jakarta` 都只做格式化（见 AD 前文的现状盘点）。判定用时区须集中一处实现，
       不要在两个地方各写一遍换算；
     - **第 3 天起（含）首次打开**即触发，**错过第 3 天不作废**。⚠️ 写成「仅第 3 天当天」即为违规：
       那天没打开 App 的用户会永远收不到，与本 FR 要解决的问题相反；
     - 已填手机号者**永不提示**（服务端 `users.phone` 非空即跳过，天然去重）；提示过即用掉，
       填了或跳过都算。
     ⚠️ **「全局仅一次」在按设备存储下只能是「本设备仅一次」**（Rule 1/2 已定标记位按设备存），
     换设备会再提示一次 —— 这是既定代价，非缺陷；已填手机号者不受影响，故实际影响面很小。
...
| **埋点回写** | 本版本埋点须并入全局清单（`docs/reference/analytics-posthog-tracking.md` 与规划侧埋点文档），避免两处口径分叉（V1.1.2 已建立该惯例） |
...
| **⚠️ 埋点文档以仓库版为准** | ✅ 该过期副本已于 2026-08-10 用仓库版覆盖（X-16 已闭合）。本版本埋点回写仍一律以仓库 `docs/reference/analytics-posthog-tracking.md` 为准，并新增一节 |
...
| 事项 | 为什么不在这里定 |
|---|---|
| ~~高度护栏的**露出余量最终值**~~ | ✅ **已闭合**（2026-08-18 Story 3.3 小屏实机复核）：定为 **40**，条目其余部分 **142**，详见 AD-6 Rule 2 |
| 裁剪交互用第三方库还是自研 | 属 story 内的实现选择，不影响任何跨 story 口径；项目现无裁剪库 |
| ~~加载期占位的**具体策略**~~ | ✅ **已定**（Story 3.3）：**有尺寸走真实比例（零跳动）、无尺寸按 1:1 预留 + 解码后调整**。跳动幅度因 clamp 而有硬上下界 |
...
| # | 事项 | 阻塞什么 | 何时闭合 |
|---|---|---|---|
| ~~**OA-1**~~ ✅ | 高度护栏露出余量的实机值 | ~~阻塞 FR-71 / FR-93 的 L2 视觉验收~~ **已解除** | ✅ **2026-08-18 Story 3.3 闭合**：余量 **40**、其余部分 **142**（口径见 AD-6 Rule 2）。360×640 小屏与 411×914 常见屏各验一遍：小屏上下一条顶部露得出来、且不劣于改版前；常见屏上 3:4 竖图完全不裁 |
| ~~**OA-2**~~ **已闭合** | 用户协议是否已覆盖"手机号用于运营召回与后续触达" | ~~阻塞 FR-70 上线~~ **已解除** | ✅ **2026-08-20 产品确认已解决，另有新版条款文档**。依据亦见源目录 PRD FR-70「用途告知与同意口径（2026-08-05 产品确认）」：用途与处理方式**已在用户协议 / 隐私条款覆盖**，故填写入口不再额外展示同意勾选或用途说明。⚠️ **新版条款文档的路径尚未登记** —— 上线前请把它挂进本行，否则日后无从追溯"依据哪一版条款" |
...
| **OA-5**（**2026-08-18 收窄**） | **FR-85 与 FR-70 的时机模型与 PRD 分叉**（2026-08-11 识别）：PRD §2.8 的「原生弹窗机会归属」整段建立在第一代双时机之上，而第二代「首启即申请」已于 2026-08-07 上线；FR-70 软引导声明复用的那两个时机在线上已不再触发 | 🟡 **已收窄，不再阻塞两个 Epic 开工**。技术口径**已由 AD-14 全部定死**（8 条 Rule + 4 点订正：所有触发点只走跳系统设置、四键物理隔离、存量不迁移、兽医端无标记位、串行顺序、埋点可线上告警校验），且 FR-85 的四个触发点时机 PRD 本就写明、并未失效 —— **作废的只有「原生弹窗 vs 跳设置」那一段分流**。<br>✅ **`[2026-08-20 硬阻塞解除]`** 唯一还挡着的 7-2（FR-70 软引导）已由产品给出新时机：**用户第 3 天打开 App 时提示**（决策 X-21，口径四点已写入 AD-14 Rule 8）。**Epic 7 与 Epic 8 现已全部可开工。**<br>✅ Epic 8 全部、Epic 7 的 7-1（字段/保存/清空/脱敏）本就不受阻。 | ① ~~产品给出 7-2 的新时机口径~~ ✅ **2026-08-20 已给（X-21：第 3 天打开时）**；② ~~产品在 AD-14 的改写口径上确认签字~~ ✅ **2026-08-21 已确认**（X-21 四点口径逐条确认）；③ **源目录 PRD §2.8 原件回写**（归档副本 `v1.1.6/PRD-v1.1.6.md` 已插入订正块）—— ②③ 属文档同步，**不挡写代码** |
