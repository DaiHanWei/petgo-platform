...
docType: 'epics-delta'
baseline: '_bmad-output/planning-artifacts/v1.0.0/epics.md'   # V1.0 冻结基线；本文件只写 V1.1 增量
inputDocuments:
...
## Epic 8: 首页情绪彩蛋（纯前端）〔❌ 已取消 2026-07-14〕

> **【已取消 2026-07-14 · 用户决策】** 首页情绪彩蛋功能整体取消，不实现。Story 8.1 不做（此前 backlog，无任何代码）。
> 保留以下原始规划仅作可追溯留痕；sprint-status-v1.1.yaml 中 epic-8 / 8-1 已置 `cancelled`；架构 DESIGN.delta 组件⑥情绪彩蛋气泡同步作废。

### ~~Story 8.1: 情绪彩蛋气泡~~（已取消）
