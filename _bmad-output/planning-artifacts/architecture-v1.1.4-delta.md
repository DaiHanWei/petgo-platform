...
scope: 'V1.1.4 增量：内容顶置、手机号采集、Feed 图片展示规范化、单条内容分享卡、用户标签、内容装饰标签、里程碑 S/M 通知补齐、多触发点推送权限唤醒、宠物主页对外分享（H5 改版 + App 内访客只读）、Feed 通栏化与首页互动、埋点 E-1~E-27'
status: final
created: '2026-08-11'
updated: '2026-08-11'
...
sources:
  ...
  - _bmad-output/planning-artifacts/v1.1.4/PRD-v1.1.4-admin.md
  - _bmad-output/planning-artifacts/v1.1.4/ui-v1.1.4.html
  - _bmad-output/planning-artifacts/v1.1.4/ui-h5-card-v1.1.4.html
  - _bmad-output/planning-artifacts/v1.1.4/README.md
companions:
  - _bmad-output/planning-artifacts/architecture/architecture-v1.1.4-2026-08-11/.memlog.md
...
> **形态说明**：V1.0 架构、V1.1 delta、V1.1.2 delta 均已上线冻结。本文件是 brownfield **增量 delta**，只写 V1.1.4 的**新增 / 取代 / 补齐**；未提及处一律**继承基线**。冲突序：**本 delta > V1.1.2 delta > V1.1 delta > V1.0 基线 > 原型**。正确性以 `v1.1.4/PRD-v1.1.4.md` 为准，App 视觉以 `v1.1.4/ui-v1.1.4.html`（**28 屏 / 8 泳道**）为准，H5 视觉以 `v1.1.4/ui-h5-card-v1.1.4.html`（**4 屏**）为准，后台以 `v1.1.4/PRD-v1.1.4-admin.md` 为准。
>
...
| **AD-14** | FR-85 触发计数的载体与迁移 | **四键物理隔离走本地偏好**；**存量旧标记不迁移、新模型不读它**；兽医端无标记位 | 用户 2026-08-11 拍板 |
...
- **Rule:**
  ...
  6. **时间窗判定必须是唯一实现**（一个共享判定方法或同一 SQL 片段），由**四处共用**：顶置的 Feed 取数、用户标签取数、装饰标签取数、**以及后台列表展示的「待生效 / 生效中 / 已结束」**。不落状态列意味着后台也要自己算——若各写一遍，左闭右开的边界会在某处被写成全闭，出现「App 已失效但后台仍显示生效中」。
...
- **Rule:**
  ...
  6. **护栏（可线上告警校验）**：触发点 4 **永不唤起原生系统弹窗**，无论用户此前是否响应过——一律 App 内引导条 + 跳系统设置。原生弹窗机会（系统只给一次）保留给触发点 1 / 2 / 5 中最先到达者。埋点中一旦出现「通知中心 + 原生弹窗」组合即为实现违背规格。
  7. 同一时机同时命中推送权限与手机号软引导时：**先推送权限、后手机号**串行处理，不叠加同屏。
  8. 手机号软引导维持「首次问诊后 或 建档后取最早、**全局仅一次**」，**不跟随** FR-85 的多触发点模型。
...
- **Rule:**
  ...
  1b. 身份证导出**已在用"按目标画布宽度反算截图倍率"**这一手法——正是本 AD 第 4 条二维码"按导出分辨率算边长"所需的同一做法，**照抄，不要另起一套**。身份证的水印做法（水印挂在截图区域的兄弟节点上，故预览与用户手动截屏带水印、高清导出不带）亦可参照。
