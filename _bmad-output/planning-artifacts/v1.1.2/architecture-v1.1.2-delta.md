...
companions:
  - _bmad-output/planning-artifacts/v1.1.2/.memlog-architecture-2026-07-31.md
...
| **AD-8** | 落地 Tab 的判定位置与持久化 | **前端在 Splash 完成回调实时计算**，不持久化「上次落在哪」。`[订正 2026-08-04]` 矩阵改为「只有已建档者落 Diary，游客例外」——**A·未建档由 Diary 改 Discovery** | 架构裁定 + 产品 2026-08-04 拍板 |
| **AD-17** | 超时兜底落地的迟到纠正（FR-91） | **兜底态是中间态而非终态**：`.timeout` 只停等待不取消请求，恢复晚到仍写回状态。故以「恢复 future 完成」为时机重判一次落地目标；🔴 **用户已自行离开则一律不纠正**（优先级高于纠正），判定须比对**路径**；兽医角色隔离守卫原样保留作双保险 | 产品 2026-08-04 拍板（闭合 OQ-23） |
...
- **Rule：**
  ...
  3. ⚠️ **过滤范围严格限定为「他人可见」的视图。作者自视视图一律不过滤** —— 成长档案时间线（FR-82）、日历与当天详情（FR-84）、「我的发布」列表均属作者自视，**必须完整展示含私密在内的全部内容**。
     ...
     - ⚠️ **第三类：作者主动授权的对外展示 → 不过滤**（OQ-18，2026-08-03 产品拍板）。**宠物名片 H5（FR-14）即使包含作者手动设为私密的 Diary，仍照常展示。**
       - 理由：`visibility` 开关约束的是**平台的自动分发**（Feed / 聚合 / 公开位），不约束**用户自己按下分享键**的行为。H5 是作者主动生成并分享出去的对外页面，属自主授权。
       - **反例（必须避免）**：某 story 按 Rule 3 的「他人可见即过滤」把 H5 也加上过滤 → 作者分享出去的名片缺内容，且他无从知道为什么。
       - 三分法总结：**作者自视 → 不过滤；平台自动分发 → 过滤；作者主动分享 → 不过滤。**
...
- **Rule：**
  ...
  2. 判定点在 Splash 完成回调（复用既有分流顺序：pending 深链 > 兽医直达工作台 > 其余）。
     `[订正 2026-08-04 · Story 7.4]` 矩阵收敛为「**只有真正建了宠物档案的人才落 Diary，游客是唯一例外**」：
     游客 → **Diary**（FR-80 种草页）｜状态 A **已**建档 → **Diary**｜**状态 A 未建档 → Discovery**（原写 Diary，已改）｜状态 B / C → **Discovery**｜兽医 → 工作台（不变）。
     实现落点：`user_state.dart` 的 `AppUserState.landingLocation`（六态穷尽 switch，落地分流与埋点 `user_state` 同源，判定顺序只写一处）。
     状态切换时点：改宠物状态那一刻落地页**不变**，须**建档完成**（转 A·已建档）后才变 Diary。
     已知代价：A·未建档的建档引导（FR-81）不再是落地页、降级为需主动点 Diary Tab 才见，建档转化率可能下降 —— 上线后按 PRD OQ-24 观测。
...
### AD-17 — 超时兜底落地的迟到纠正（FR-91）

- **Binds：** FR-91；FR-89 的 5s 兜底；AD-8 落地矩阵。
- **Prevents：** 慢网用户被永久留在错误落地页 —— 尤其 **B/C 会停在 Diary 的「有宠专属」功能拒绝页**，而这正是 FR-78 把 B/C 分流去 Discovery 所要避免的体验。
- **技术根因（先读这条，否则会误判为"多余的补丁"）：**
  `ensureRestored()` 的 `.timeout()` **只停止等待、不取消底层请求**。恢复请求继续跑，完成后写回 `AuthState`。
  所以**超时兜底送去的落地页是会变化的中间态，不是终态**。
  - **兽医能自愈** —— `redirect` 里有角色隔离守卫（`if (auth.isVet)`），会把它收口回工作台；
  - **已登录的 A·未建档与 B/C 不能** —— 门控条件是 `!auth.isLoggedIn && controlled`，已登录不命中；`redirect` 里**没有任何分支**会把已登录用户从 Diary 移到 Discovery。
- **Rule：**
  1. **仅当本次冷启动的落地由超时兜底产生时**启用。正常路径（恢复在动效结束前完成）的落地已是正确结果，不重判。
  2. 触发时机为**恢复 future 完成**（成功或失败都算完成），**不是固定延时** —— 固定延时要么早于恢复（白跳）要么晚于恢复（用户已在操作）。
  3. 🔴 **仅当用户仍停在兜底送去的那个落地页上才纠正。**「把正在浏览的人拽走」比「落错页」严重得多，**本条优先级高于「纠正到正确页」**；两者冲突时以本条为准。
     ⚠️ 判定**必须比对路径**，**不得**使用「路由栈深度」之类的间接信号 —— 用户在同页内打开弹层 / 进详情再返回，栈深会变但人没走，会被误判成"已离开"而放弃本该做的纠正。
  4. **只纠正一次**（一次性额度），此后本次冷启动内不再干预，防止与其它跳转逻辑互相拉扯。
  5. 本次冷启动若由**深链唤起 → 不纠正**（深链优先级最高，与 AD-8 分流顺序一致）。
  6. 恢复**失败**（确实是游客）→ 重判结果仍是兜底目标 → **不产生跳转**，不打扰真游客。
  7. 🔴 **兽医角色隔离守卫原样保留、不得被本机制替代** —— 那是用户/兽医路由互斥的**安全边界**，继承「安全规则只升不降不可绕过」。两者算出同一目标，一次性额度保证只跳一次。
- **可观测性：** 兜底落地与纠正后落地**各上报一次既有 T-1 事件** `app_launch_landed_on_tab`（**不新造事件名**，否则 Story 6.1 已交付的看板口径会断），分别带 `restore_timeout: true` 与 `corrected_from: <原目标>`。这使「超时兜底实际发生率」成为可观测指标 —— PRD **OQ-23 因此闭合**，不再是只能推测的开放问题。
- **代价（产品已接受）：** 纠正会产生**一次可见跳变**（与兽医现有行为同性质）。判断依据是「一次跳变优于永久停在无效页」。视觉稿未画该过场，按普通路由跳转实现。
- **实现落点：** `app_router.dart` 的 `LateLandingCorrection`（额度状态）+ `resolveLateCorrection()`（纯判定，可单测）+ splash `onComplete` 内的武装逻辑。
- **⚠️ 反向约束：** `.timeout()` **不得**改成可取消的实现 —— 恢复晚到就不会写回 `AuthState`，本机制将永不触发。`app_router.dart` 已就地加注释，测试亦有源码级锁定。
...
| OQ | 事项 | 架构影响 | 阻塞级别 |
|---|---|---|---|
| ~~OQ-18~~ | 手动关私密的 Diary 是否仍出现在主动分享的名片 H5 | 无 | ✅ **已闭合（2026-08-03 产品拍板）**：**仍展示，H5 不过滤**。已写入 AD-4 Rule 3 的三分法 |
