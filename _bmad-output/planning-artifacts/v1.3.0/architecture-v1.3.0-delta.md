...
- **Rule:**
  ...
  3. **本批次聚合页只有两张卡**：KTP、年龄换算。护照（FR-120）与 Tailsonality（FR-117）**不占位、不置灰、不出现** —— 批次 C 上线时是**新增卡**，不是解锁已有占位。实现不得预埋隐藏卡位。
...
- **Binds:** FR-65 迁移引导蒙层、批次 C 第二次触发
- **Prevents:** 每来一个一次性引导就往用户表加一个 boolean 列；以及本批次与批次 C 的两次触发共用同一个标记（PRD 明确是两次独立触发）。
- **Rule:**
  ...
  2. ✅ **按账号存，不按设备**（Dai 2026-09-10 拍板，OQ-7 已闭合）。⚠️ 这与 v1.1.6 **AD-14 第 1 条**（一次性引导标记走本地 prefs、按设备不跟账号）**不同**。
     ...
     - **两条并存，各管各的场景，不得互相套用。**
  3. **本批次只落第一个键**（KTP 位置迁移）。批次 C 的 Tailsonality 入口引导**另起一个键**，**禁止共用**。
...
| # | 事项 | 为何不定 | 重访条件 |
|---|---|---|---|
| D-1 | 庆祝页与徽章**视觉重绘**、38 枚插画产制 | 属批次 C（`v1-3-0prd-C.md`）。本批次交付后视觉维持现状 | 批次 C 开工 |
| D-2 | 聚合页第 3、4 张卡（护照 FR-120 / Tailsonality FR-117） | PRD 明令本批次不占位、不展示 | 批次 C 开工 |
| D-3 | 迁移引导蒙层**第二次触发**（Tailsonality 入口） | 属批次 C；键已按 AD-A21.3 隔离 | 批次 C 开工 |
...
| D-11 | **第三个及以后的一次性引导该走哪套标记** | 本批次只定了这一个键，且明令与 v1.1.6 AD-14 的按设备模式「不得互相套用」（AD-A21.2）。第三个引导出现时才有足够信息定归属 | 批次 C 落第二个键时一并评估是否需要一张统一的会合表 |
...
| **R-8** | 源目录 `v1-3-0prd-C.md`（frontmatter `batch: B2`）+ 母版 `1-3-0prd.md` + `v1-3-0prd-A.md` | 🔴 **「批次 B2」这个叫法已取消，统一为「批次 C」**（Dai 2026-09-10）。源文件里 `prd-C.md` 的 frontmatter 仍写 `batch: B2`，母版与 PRD-A 正文仍称「批次 B2」，配套 UI 稿却已叫 `ui-batch-c-*` —— 同一批东西三种叫法。**仓库内 43 处已统一为「批次 C」，源文件待产品侧同步** |
