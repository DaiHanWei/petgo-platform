...
- **UX-S1** 后台页面一律用 admin 主题的模板 A~E（A=工作台 B=列表 C=报表 D=配置卡 E=分步）。🔴 **2026-09-16 更正**：htmx 局部更新（200 主 fragment + `hx-swap-oob`、422 行内错、403 禁用态注明缺哪个权限）**只适用于模板 A/B**；**模板 D/E 与所有整页表单维持现状 PRG，不混用**（admin AD-9 原文）。商品表单页是模板 D ⇒ SHOP-FR-20 的即时校验须按 PRG 实现或先与 admin 对齐。各页模板归属见架构 delta §3 更正表。
...
- **UX-S3** banner 顶栏三段遮罩对比度**每一张图都要满足**，不能只验第一张。🔴 **2026-09-16 更正**：9.09:1 是某一张实测图的测值、不是通用门槛；代码里真正在用的是标题/大字 ≥3:1、胶囊 ≥4.5:1 —— 口径实施前澄清，「每张都验」的要求不变。
- **UX-S4** 批量操作的二次确认走后台模板的确认态，不是浏览器 `confirm()`。
- **UX-S5** 所有导出走 admin AD-10 的 `AdminExportWriter`（已含公式注入防护），禁止各页自拼字符串。⚠️ 全仓现有导出**一个都没做公式转义**，本主题是首次落地。
...
## 2026-09-16 代码核实更正（权威，优先于上文）

40 个 story 生成时逐条 grep 核实，共发现 23 条本文与代码现状不符之处，**完整表格见架构 delta `architecture-v1.3.0-shop-v2-delta.md` §9**。对拆 story 影响最大的六条：

1. **批量改库存不能循环调既有盘点方法** —— 它内部重读前值，会让「预览即契约」变成假保护（7-3）。
2. **banner 改多图会让存量 App 首页横幅静默消失** —— 老端点必须保留，多图另开复数端点（8-6）。
3. **后台模板字母原表四个页面全标错**，已按 admin 真实定义更正；且**模板 D/E 维持 PRG 不混用 htmx**，直接影响 SHOP-FR-20 的实现方式（8-5）。
4. **加权限码实为 6 道关卡**，其中预置角色两表本分支尚不存在 —— **不要为凑满而造表**（6-2、7-1、9-3、9-4）。
5. **「品类超过 4 个改横向滑动」App 侧早已实现**，该条降级为验证 + 换数据源，省一块工作量（6-3）。
6. **本仓没有合格的批量操作先例**，现有三个都是反例（只记汇总审计、无上限、部分失败不回滚）—— 7-1 是首个，禁止照抄。
