...
companions:
  - _bmad-output/planning-artifacts/v1.1.4/.memlog-architecture-2026-08-15.md
...
- **筛选是统一视图成立的前提，不是锦上添花**：按工单类型（三类）+ 按状态（**待处理 / 已处理 / 无需处置**，第三档取中性词见 C-103）+ 按被举报账号检索，三者必须与列表同版本交付。
...
| ② **终态无直接对应** | name 的终态是 `RESOLVED_PASS` / `RESOLVED_VIOLATION` 两个；avatar 只有单一 `RESOLVED` | AB-3D 的三态筛选（**待处理 / 已处理 / 无需处置**）对第三类做映射：`MANUAL_PENDING → 待处理`；name 的 `RESOLVED_VIOLATION` 与 avatar 的 `RESOLVED`(verdict=VIOLATION) → **已处理**；name 的 `RESOLVED_PASS` 与 avatar 的 `RESOLVED`(verdict=PASS) → **无需处置**。**该映射须在实现时固化并写进注释** |
...
| # | 问题 | 责任人 | 阻塞什么 |
|---|---|---|---|
| ~~**OQ-A4** 🚩~~ ✅ | ~~封号通知印尼语措辞待**法务**确认~~ **法务已审核通过（2026-08-16，C-101）** —— 「引导联系客服反馈异议」不构成承诺正式申诉结果的风险。**上线阻塞解除，封号功能可随版本正常发布**。<br>⚠️ 仍有两条**印尼语待运营产出**（警告通知 · FR-51 改后的回告），**已降级为不阻塞发版的普通待办**，上线前收口即可（「提交后无法撤销」与「解除拉黑第二句」UI 稿已有草稿） | 运营 | ✅ 不再阻塞上线 |
| ~~**OQ-CA1**~~ ✅ | ~~账号标识字段审核工单在统一队列里的优先级分怎么定~~ **产品已确认（2026-08-16，C-102）**：取 **`HIGH → 10` / `NORMAL → 2`**，锚点取自 PRD 自己的两个例子。⚠️ **`NORMAL` 不得映射成 0**（默认值，映射成 0 会让这类工单几乎全部永远沉底）。**规格已回写 PRD §4.1 正文** | 产品 | ✅ 已解决 |
| ~~**OQ-CA2**~~ ✅ | ~~第三类工单的三态映射需产品确认~~ **产品已确认（2026-08-16，C-103）**：分类逻辑按「有没有对被审对象采取动作」保持不变，但**第三档标签由「已驳回」改为中性词「无需处置」**（`Tidak Perlu Tindakan` / `No Action Needed`）—— 「已驳回」只对举报成立，第三类没有请求方。⚠️ **只改状态筛选档标签，不改「驳回」这个动作按钮**；**不新增第四档「已通过」**（举报类在该档永远为空）。规格已回写 PRD §4.1 正文 | 产品 | ✅ 已解决 |
