...
| `PRD-v1.4.0-后台.md` | 运营后台 PRD，**新建模块 10–13**（商品与库存 / 订单履约 / 退货与售后 / 电商经营数据）+ **模块 6 扩展**（AB-6D 新增、AB-6A 第二条溢价、AB-6C 口径变更） | `v1-4-0电商后台prd.md` |
...
## 4. ~~⚠️ 落库时核实：5 处「PRD 假设 vs 当前代码」偏差~~ → **已承接，本节退化为历史记录**

> ✅ **2026-08-14 `bmad-prd` validate 已正式承接这五条**，按本文件 §11 的预期，本节自此仅作历史记录保留，**不再是台账**。当前有效位置：
>
> | 本节 | 正式编号 | 现在住在哪 |
> |---|---|---|
> | ① 混合支付冲突 | **L-8** + **D-9 / OQ-35** | `decision-log.md` §三 · PRD §8A · FR-100 就地修正 |
> | ② 无体重字段 | **L-9** | `decision-log.md` §三 · PRD §8A · §3.9「前置数据资产」 |
> | ③ 无周期/到期字段 | **L-10** + **D-10 / OQ-36** | `decision-log.md` §二/§三 · PRD §8A · FR-108 就地标注 |
> | ④ 单宠物硬约束 | **L-11** | `decision-log.md` §三 · PRD §8A · FR-93 已改写 |
...
~~**待架构 delta 拍板（二选一）：**~~ → ✅ **2026-08-15 拍板 (a)，见 `decision-log.md` C-10 与 PRD FR-100**
- **(a) 扩展 `PaymentIntent`** ← **已选**：`PayChannel` 末尾追加 `MIXED`，新增 `coin_amount` / `cash_amount` / `coin_ratio` 三列（nullable，既有 4 个 purpose 全 NULL、行为不变），退款在同一笔 intent 内按 `coin_ratio` 拆分 → 天然原子。⚠️ 改动共享表，Flyway 须占独占号段 + 事先告知另两位同事
- ~~(b) 电商订单侧自建支付拆分表~~ —— 未选（隔离性好，但订单-支付一对多、对账链路变长）
...
~~**需产品拍板（二选一）：**~~ → ✅ **2026-08-15 拍板 (a)，见 `decision-log.md` C-11**
- **(a) FR-108 依赖 1.2.0，本版本不做** ← **已选**：整体挪 Roadmap 1.2.0，`health_records` 零 schema 改动，UX 不出相关屏，埋点不埋相关事件
- ~~(b) 本版本自带犬猫驱虫/疫苗常规间隔默认值~~ —— 未选
- 🔴 **代价：** 复购引擎由三条降为 FR-107 + FR-109 两条，冗余归零 → **DEP-6 升为「阻塞版本核心论证」**
...
| ~~D-5 / OQ-29~~ | ~~复购触发卡与健康提醒是否合并为一条推送~~ | ⏸️ **08-15 顺延 1.2.0**：C-11 拍板 FR-108 挪走后本版本无健康记录触发推送，冲突消失 | ~~FR-108 推送模板~~ |
...
| 🔴 **DEP-6** | 首批 SKU 清单、进货价、**每日建议喂量数据**（结构化，非自由文本） | Rendy | 🔴 **版本核心论证**（08-15 二次上调）——C-11 把 FR-108 挪走后复购引擎冗余归零，这份数据是 FR-109 的唯一计算依据。**须在商品录入排期前拿到** |
...
仓库现有 Flyway 最高到 **V100**（2026-08-15 实测：99 个文件、**V86 为空洞**、`main`/`stag`/`hex/v1.1.4`/`hex/v1.1.2` **四条分支全部停在 V100**）。V1.4.0 新增迁移按 §6 护栏 1 划定的**独占号段**分配，**不是简单从 V101 顺延**。

✅ **2026-08-15 已落：** 号段提案 **V101–V139** 归本工作线，规划见 `architecture-v1.4.0-delta.md` **§4**；纪律见 `implementation-artifacts/v1.4.0/PARALLEL-DEV-CONTRACT.md` **§1**（**待三人签字**，Hex / george 的号段边界待他们自己填）。

> **V86 的空洞恰好证明这条约束可行：** Flyway **从不要求版本号连续**——按号排序依次执行，跳号完全合法。所以独占号段不需要任何额外机制。**同时并行下必须废掉 E2 的「merge 时号继续单调顺延」**：重排会让已在 stag/测试库应用过的迁移校验和失配，Flyway 直接启动失败。独占号段的全部意义就是让重排永远不必发生。
...
1. ✅ **`bmad-prd`（validate 意图）—— 2026-08-14 已完成。** 三名评审员（rubric walker + adversarial-general + 跨文档一致性）共 **128 条 findings**，Grade **Poor**（无 broken 维度，是 7 条 critical 拉下去的；`substance over theater` 判 **strong**）。§4 五处偏差已正式承接为 **L-8~L-12**，critical + high 已回写进两份 PRD 与 `decision-log.md`（新增 D-9/D-10 · OQ-35~OQ-42 · SPEC-1~SPEC-25 · DEP-9）。报告：`../prds/prd-petgo-platform-2026-08-14/`。原操作说明见 §11（保留备查）
   > ~~**进入第 2、3 步前必读：** OQ-35 与 OQ-36 不闭合，UX 会踩空、架构 delta 无从落笔~~ → ✅ **2026-08-15 两条均已拍板，见下方 1.5**

1.5 ✅ **2026-08-15 产品拍板 4 项，两处 chain-blocker 全部解除：**

   | 原编号 | 结论 | 落为 |
   |---|---|---|
   | D-9 / OQ-35 / **L-8** | **扩展既有 `PaymentIntent`**（方案 a）——`PayChannel` 末尾追加 `MIXED`，新增 `coin_amount`/`cash_amount`/`coin_ratio` 三列（nullable，既有 4 个 purpose 全 NULL、行为不变）；退款在同一笔 intent 内按比例拆分 → 天然原子 | **C-10** · PRD FR-100 · 后台 AB-12C |
   | D-10 / OQ-36 / **L-10** | **FR-108 整体挪 Roadmap 1.2.0**（方案 a）——不自建间隔默认值，`health_records` 零 schema 改动 | **C-11** · PRD §3.9 / §7 |
   | OQ-38 | **支持行级部分退货**，同订单同时仅一张进行中申请；**整单退→去程运费全退，部分退→不退**；部分退不改订单主状态 | **C-12** · 新增 **FR-104A** · §4.1/§4.2 · 后台 AB-12A |
   | OQ-39 | **砍掉换货**——枚举收为 `可退`/`开封不退`/`不可退`；§3.7 与后台模块 12 更名；「换货」写进 §7 | **C-13** |

   **连带自动消解：** D-5/OQ-29（推送合并冲突，顺延 1.2.0）· A-16 疫苗类分母污染 · SPEC-15（提前量定 7 天，随 FR-108 顺延）。

   > 🔴 **C-11 的代价必须被排期看见：** 复购引擎由三条机制降为 **FR-107 + FR-109 两条，冗余归零**。FR-109 的唯一计算依据是 **DEP-6**（每日建议喂量数据，Rendy），**已二次上调为「阻塞版本核心论证」**——须在商品录入排期前拿到，不能「录商品时顺便补」。若确认拿不到，应回头重议 C-11（改用驱虫间隔默认值把 FR-108 救回来），而不是让复购引擎带着单点依赖上线。

2. `bmad-ux`（**CU**）—— 17 屏已覆盖主链路，本步重点是补异常态/空态并对齐 v1.1.2 设计 token。**08-15 更新：** 结算页两段金额与退款方式拆分已随 C-10 定稿可画；**FR-108 相关屏（Diary 内嵌 CTA、健康提醒推送附商品）本版本不出**；新增需画**行级部分退货的勾选与去程运费提示**（C-12）
3. `bmad-architecture`（**CA**）—— ~~先定混合支付模型~~ → 模型已定，本步任务变为**把方案 a 落成 AD 条目**（`MIXED` 枚举契约、三列可空性与不可变性、`coin_ratio` 精度与舍入——拆分金额须整数分账不产生 1 盾漂移），**然后定 §6 并行护栏**（Flyway 独占号段 · 共享枚举与 `OrderCenterService` 追加约定，须三人共同确认）
4. `bmad-create-epics-and-stories`（**CE**）→ `bmad-check-implementation-readiness`（**IR**）→ `bmad-sprint-planning`（**SP**）

**并行启动（不等上述链条）：** 🔴 **DEP-6 催 Rendy（已升为最高）** · DEP-1 与 Tab 改版负责人对齐 · DEP-3 / DEP-4 / DEP-7 的法务与商务前置。

**仍待拍板但不阻塞后端开工：** OQ-37（不用 PawCoin 开关）· OQ-40（PPN/发票）· OQ-41（注销级联与 UU PDP，**安全攸关**）· OQ-42（kill criteria）· 后台 OQ-30/31/32。
