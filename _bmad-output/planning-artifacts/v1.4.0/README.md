...
| ✅ ~~DEP-6~~ | 首批 SKU 清单、进货价、**每日建议喂量数据** | **本人**（08-17 接手，原 Rendy 离职） | ✅ **已解除**：喂量印在商品包装上，录入时照抄；SKU 清单已有初步版本 |
...
1.5 ✅ **2026-08-15 产品拍板 4 项，两处 chain-blocker 全部解除：**
   ...
   **连带自动消解：** D-5/OQ-29（推送合并冲突，顺延 1.2.0）· A-16 疫苗类分母污染 · SPEC-15（提前量定 7 天，随 FR-108 顺延）。
   ...
   > ✅ **2026-08-17 更新：C-11 的代价已大幅缓解。** 复购引擎仍是 FR-107 + FR-109 两条，但 **DEP-6 已解除** —— 喂量数据印在商品包装背面，录入商品时照抄即可，不再依赖外部交付。**无需回头重议 C-11。**
   > ⚠️ 仍保留两条监控：AB-13B 的「FR-109 触发覆盖率」（读出运营是否真填了）· Story 1.3 的 Makanan 留空警告（数据可得后，留空从「客观受阻」变成「主观跳过」）。
...
**并行启动（不等上述链条）：** ~~🔴 DEP-6 催 Rendy~~ ✅ **已解除（08-17 本人接手，喂量数据印在包装上）** · DEP-1 与 Tab 改版负责人对齐 · DEP-3 / DEP-4 / DEP-7 的法务与商务前置。
