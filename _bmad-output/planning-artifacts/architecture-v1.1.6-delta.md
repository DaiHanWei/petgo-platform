...
| **OA-6** | **App 内访客只读视图是否也要套拉黑过滤**（2026-08-12 新识别）：查漏补做清单 A-1 的表格只裁定了 H5（无登录态、没有"当前用户"，FR-94 已明确排除），但**App 内访客视图对已登录用户开放**——若我拉黑了某人、朋友把他宠物的链接转给我，我点开还看得到吗？该场景清单未覆盖 | 不阻塞开工；阻塞的是口径确定性 | 建议判为**不过滤**，两条理由：① 访客视图是用户**主动点开**的定向分享，不是平台分发，与"我不想在信息流里看到这个人"不是同一件事；② **不过滤是零成本的默认行为** —— 现有过滤层长在 Feed 主查询里且由"是否登录"门控，而 AD-1 规定访客视图走**独立投影、不复用现有查询**，故它**天然不继承**该层，"过滤"反而是需要特意加的工作（2026-08-14 代码实读佐证；**2026-08-16 V1.1.4 落地后复核，此结论不变**——新增的拉黑子查询同样长在 `findFeed` 里、同样由 `hasViewer` 门控，访客视图依旧不继承）。⚠️ **2026-08-16 新增参照点，会影响这条怎么判**：V1.1.4 已就"拉黑后能不能进对方主页"做过裁定——**主页访问要过滤**，且是其六处生效点中**唯一区分 source 的一处**（只认 `BLOCK` 主动拉黑，`REPORT` 举报隐藏照常放行，因为「已举报」状态与重复举报入口都依赖能进得去）。这与本条"访客视图不过滤"的建议**方向相反**，而两者语义相邻（都是"点开某人的页面"）。产品定夺时须明确：宠物主页的访客只读视图，是算"主页访问"（跟随 V1.1.4 过滤、且只认 BLOCK），还是算"定向分享链接"（不过滤）。待产品确认 |
