...
>
> **跨版本关系（三条，全部是单向、无环）**：
> - **本版本 → V1.1.4**：唯一一处向后依赖 —— 顶置坑位须套用 V1.1.4 FR-94 的拉黑过滤层（AD-8 Rule 6b）。V1.1.4 先发，属合规的向后引用。
...
- **Rule:**
  ...
  6b. **🔴 顶置内容必须套用拉黑过滤**（2026-08-12 补入，落地查漏补做清单 A-1）：若顶置内容的作者已被**当前查看者**拉黑（V1.1.4 FR-94），则对该查看者**视为坑位为空**，按上方回退规则由普通内容填充；**不为该用户单独选替补顶置内容**，其他用户不受影响。
      ...
      - **该层的确切现状（2026-08-14 代码实读）**：Feed 主查询（`ContentPostRepository.findFeed`）里**已有**一个「按查看者隐藏」的相关子查询，但**目前只过滤举报**（排除当前查看者已举报过的帖，内容审核 cm-6 §5.4），**尚无拉黑**。该子查询由 `hasViewer` 布尔标志门控——**游客不过滤**（且刻意用布尔标志而非裸 NULL 判断，以避开 PG 无法推断 NULL 参数类型的 42P18）。V1.1.4 的拉黑应在此处并列增加一个判断，本版本顶置作者查询套同一处。
      - 🔴 **前置条件目前尚不存在**：**用户拉黑（FR-94）在代码里一行都没有**，V1.1.4 尚未开工（2026-08-14 核：main 停在 08-09，无相关分支）。**这是一个真实的外部阻塞，不是文档问题** —— Epic 4 的 A-1 补丁必须等 V1.1.4 的拉黑表与过滤落地后才能实现。
      - ⚠️ **这是本版本对 V1.1.4 的唯一一处向后依赖**（V1.1.4 并行开发且**先于本版本发版**）。若 V1.1.4 的实际发版晚于本版本，A-1 无处可套，**须回头重新评估**（选项：本版本自建过滤 / 顶置暂不做拉黑过滤并记为已知缺陷）。
...
| **OA-7** | **四个埋点事件名是否按规范改名**（2026-08-14 新识别，来自 `埋点清单v116.md` §1）：`phone_prompt_result` / `push_permission_result` / `push_permission_state_snapshot` / `post_share_card_share_completed` 四个名字不合既定的 `模块_对象_动作`（动作在词尾）规范，清单给了建议名 | 不阻塞开工；但**一旦上线就不能再改**——改名会切断历史序列（v112 有 9 个旧事件正是因此故意没改名） | 决策权在产品 / 工程。建议**改**，趁本版本这批事件全是新增、没有历史序列可切断，是唯一零成本的时机 |
| **OA-6** | **App 内访客只读视图是否也要套拉黑过滤**（2026-08-12 新识别）：查漏补做清单 A-1 的表格只裁定了 H5（无登录态、没有"当前用户"，FR-94 已明确排除），但**App 内访客视图对已登录用户开放**——若我拉黑了某人、朋友把他宠物的链接转给我，我点开还看得到吗？该场景清单未覆盖 | 不阻塞开工；阻塞的是口径确定性 | 建议判为**不过滤**，两条理由：① 访客视图是用户**主动点开**的定向分享，不是平台分发，与"我不想在信息流里看到这个人"不是同一件事；② **不过滤是零成本的默认行为** —— 现有过滤层长在 Feed 主查询里且由"是否登录"门控，而 AD-1 规定访客视图走**独立投影、不复用现有查询**，故它**天然不继承**该层，"过滤"反而是需要特意加的工作（2026-08-14 代码实读佐证）。待产品确认 |
| **OA-5** | **FR-85 与 FR-70 的时机模型与 PRD 分叉**（2026-08-11 新识别）：PRD §2.8 的「原生弹窗机会归属」整段建立在第一代双时机之上，而第二代「首启即申请」已于 2026-08-07 上线；FR-70 软引导声明复用的那两个时机在线上已不再触发 | 🔴 **阻塞 FR-85 与 FR-70 开工** —— 照 PRD 字面实现会做出一段永远走不到的分支（"从未响应过 → 唤起原生弹窗"），且手机号软引导会指望一个不再触发的门控 | 产品确认 PRD §2.8 改写口径（本 delta AD-14 已按线上现实先行订正，PRD 待同步）；FR-70 须自行实现时机判定 |
