...
- [ ] **B1. 引入阿里 OSS/STS SDK 与双桶配置** (AC: 1, 3)
  - [ ] `pom.xml` 加阿里云 OSS Java SDK + STS（`aliyun-sdk-oss` + `aliyun-java-sdk-sts`/`aliyun-java-sdk-core`，取与 Java 25/SB4 兼容最新版，记录实际版本）。
...
- 记录所用阿里 OSS/STS SDK 实际版本（若与 Java25/SB4 有兼容调整）。
