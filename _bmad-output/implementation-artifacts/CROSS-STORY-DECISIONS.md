...
>
> **2026-07-08 增补（CM1–CM8）**：源自《内容审核补充规范 v1.0.1》（`planning-artifacts/content-moderation-plan-v1.0.1.md`）拆解为 9 份 `spec-content-moderation-*.md`（见 `spec-content-moderation-overview.md`）。CM 系列**在 F10 之上叠加修订**（不删 F10），与既有 C/D/E/F 同级权威。
...
| **CM1** | 护栏（**修订 F10，叠加不删**）| **内容审核引入「已发布待审、仅作者可见」中间态**（内容审核补充规范 v1.0.1，2026-07-08）：内容（帖子/评论）在审核返回**通过**前一律**仅作者本人可见、他人不可见、不显示任何「审核中」标签**；通过→他人可见，未通过→维持仅作者可见并处置。**L1 强制拦截等即时判定不入中间态**，仍走 F10 原语义「发布失败+停留编辑页+提示重试」。这是对 F10「发布前同步闸门、无中间态」的**增量修订**（≥0.8 高风险/三方降级项才进挂起态并入人工队列）。stub `ContentModerationService` → 真三方（阿里云内容安全）替换，**护栏「禁新中间件」不变**（熔断/降级走应用内自研极简实现）| overview §3 D-CM1/D-CM2；`spec-content-moderation-post-review`（激活 FR-12A+可见性，改 `findMyPosts`/成长时间线 status 过滤）；`-comment-review`；`-report-triage`（P0 挂起复用）|
| **CM2** | 架构 | **审核队列 = 混合架构**：帖子+评论**共用** `manual_review_queue`（加 `content_type` 区分，**owner = 评论 story，占位 V49**）；昵称/宠物名（`name_moderation_records`，V50）、头像（`avatar_reviews`，V51）**各建独立表**（审核证据+版本键+挂起语义与帖子不同）。后台（admin-console）以**分来源多页**呈现（帖子/评论共队列页 + 名称/头像各处置页），经 story 4/5 的待审读接口消费，**非单一聚合查询** | `spec-content-moderation-comment-review`（V49 queue.content_type）；`-name-review`/`-avatar-review`（独立表）；`-admin-console` §6.1/§6.2 |
| **CM3** | 契约 | **审核类通知类型统一**（以通知 story 为准）：单一 `NAME_RESET` + `targetRef` 前缀（`user:`/`pet:`）派生变体（**不用** `NAME_RESET/PET_NAME_RESET` 双型）；`AVATAR_RESET` 同理；§8.5 评论下架 / §8.9 举报下架**复用 `CONTENT_REMOVED`** + targetRef 派生变体（不新增类型）；新增 `CONTENT_REVIEW_TIMED_OUT`（§8.8 从 REJECTED 拆出）。**类型 CHECK 扩展归通知 story（占位 V53），须先于名称/头像 story 合并**，否则通知 INSERT 撞 CHECK。App 按 `type + targetRef` 本地化，后端不下发显示串（沿用 i18n 债约定）| `spec-content-moderation-notifications-i18n`（V53）|
| **CM4** | 运营裁决 | **举报阈值分级（仅举报驱动）**：唯一举报用户数 **≥10 → P0** 自动转「仅作者可见」待判（2h SLA）；**3–9 → P1** 不自动下架、维持现状（24h）；**单次/中风险 → P2**（72h）。**单个 `ILLEGAL` 举报即触发 P0 自动挂起**（2026-07-08 拍板取「是」，从严备选未采）。举报**每人一次 + 举报即对本人隐藏** → 举报次数天然 = 去重后唯一举报用户数（D-CM7）。原「三方高风险≥0.8」P1 行已移除（提交时已由 FR-12A 拦截）| `spec-content-moderation-report-triage`（读路径 `findFeed` NOT EXISTS 过滤 + `report_hidden_at`）|
| **CM5** | 数据生命周期 | **账号维度违规计数（仅记录不处置）注销口径**：`violation_counts`（按账号+类型 POST/COMMENT/NAME/AVATAR 累加，仅计**人工判定**违规，评论 FR-55 自动同步拦截不计）属**账号维度聚合数据**，随注销按 **D1/D2 级联删除/匿名化**（跟随账号）；底层**审核证据记录**（含内容原文，方案 §5.5）**无限期留存但解除账号直连**（匿名化引用）。**三套名称/内容占位机制互不混用（D-CM4）**：①违规名重置=写真实列 `user_<hex>`/`Pet_<hex>`（`is_system_default_name=true`）②注销匿名化=显示层「已注销用户」(D1/7-3，**不动**) ③举报/审核下架=可见性隐藏 | `spec-content-moderation-account-strikes`（V55 `violation_counts`）；`-name-review`；7-3（不回退）|
| **CM6** | 语义澄清 | **两条 priority 轴勿混**：`manual_review_queue.priority`（**提交时**高风险帖/降级项入队处理紧急度排序，admin-console 占位 V54）**≠** 举报驱动 **P0/P1/P2**（**已发布**内容处置优先级，report-triage）。队列 priority 为通用「处理紧急度」，具体入队项到 P0/P1/P2 的**映射由生产者 story 2/3/6 定义**；未标则默认 P1（不沉底）| `spec-content-moderation-admin-console` §5.1/R4；`-report-triage` |
| **CM7** | 护栏 | **隐藏才通知**：仅「隐藏/违规/未通过」负向最终结果推送；「通过/保留/恢复」**不推送**。据此**删除** §8.6 帖子审核通过正向通知（`CONTENT_REVIEW_APPROVED` 发送移除、枚举惰性保留不从 CHECK 删、审计仍记）、§8.4 名称通过通知；§8.3 逾期限权通知随「重置默认名」模型作废 | `spec-content-moderation-notifications-i18n`；`-post-review`（通过静默转正）|
| **CM8** | 产品模型 | **违规名称/头像处置 = 重置为默认**（**无 7 天期限、无账号限权、无「***」屏蔽**）：违规昵称/宠物名 → 重置为**不可枚举默认编码名**（`user_<hex>`/`Pet_<hex>`）；违规头像 → 重置为平台默认头像（头像「先放行后审」的可见窗口期为已知权衡）。名称/头像/帖子/评论**编辑后重新送审**，**审核结果绑定内容版本**（version 键），若出结果时值已改则旧结果**静默作废**、队列条目移除（D-CM3）。名称/头像审核为**先放行→异步审核**（`@Async`+DB 状态机，禁 MQ）| `spec-content-moderation-name-review`/`-avatar-review`/`-post-review`/`-comment-review` |
