...
>
> **2026-06-08 增补（F1–F8）**：源自 incoming PRD V1.0.0(2026-06-06) 的 correct-course 影响评估，见 `planning-artifacts/v1.0.0/sprint-change-proposal-2026-06-08.md`（已批准）。F 系列与既有 C/D/E 同级权威。
>
> **2026-06-08 第二轮增补（F9–F15）**：源自上游 PRD「12 处逻辑补齐」提交（异常态 / 退出态 / 并发竞争 / 跨模块一致性），见 `planning-artifacts/v1.0.0/sprint-change-proposal-2026-06-08-gapfixes.md`。**F10 反转 F1 的内容审核条款**（按用户 2026-06-08「最新 PRD 决定优先」指令）。
...
| **E2** | 基建 | ~~**Flyway 序号按 dev 执行顺序单调分配**，勿照搬 architecture 示例号（示例 `V2__init_profile` 与 1.3 实占 `V2__init_auth` 会撞）；各 story 用 `V<n>__` 占位~~ **← 序号分配方式已被 E6 反转（2026-08-21，时间戳版本号）**；「已提交迁移冻结、改表另起新迁移」的精神仍有效并入 E6 | sprint-status.yaml 头注（全局约定） |
...
| **E6** | 基建（~~反转 E2 序号分配~~，2026-08-21） | ~~见下述原文~~ **← 已被 E7 取代（2026-08-21 hanwei 拍板废止）**：时间戳制本身保留并升为常设规则，废止的是 E6 的两条过渡条款 —— ②「已合入 main 的迁移冻结」（判据改为「是否已被任何环境应用过」）与④「在途分支序号迁移合 main 前须改成时间戳号」（存量序号一律保留原号，不再返工）。原文留档：详见 git 历史 `4e8931ac`。 | 已被 E7 取代 |
| **E7** | 基建（**取代 E6**，2026-08-21 hanwei 拍板） | **Flyway 时间戳制为常设规则，无过渡条款**：新迁移一律 `V<yyyyMMdd_HHmm>__<snake_case>.sql`，禁止序列号。① **存量序号迁移 V1–V108 一律保留原号，不改名不返工**（名单固化在 `scripts/ci/flyway-legacy-versions.txt`，只增不减）—— 它们多已应用到 prod / `petgo_stag`，改名会让 Flyway 找不到已应用记录而拒绝启动；**一切以数据库现状为准，树迁就库、不是库迁就树**。② **冻结判据从「在不在 main 上」改为「有没有被任何环境应用过」**：已应用的绝不能改（checksum 对不上即启动失败），从未应用过的可以直接改（本次 4 条电商迁移即据此修正）。③ CI 守门相应放宽：只对**不在 legacy 名单内**的新增文件强制时间戳格式，其余检查（树内同号 / 动 main 已有文件 / 与 main 撞号）不变。④ **打包一律 `mvn -B clean package`** —— 不带 clean 会把改名前的旧迁移留在 `target/classes` 里一并打进 jar，Flyway 报 `Found more than one migration with version X` 启动即崩（2026-08-21 实际踩过）。背景：E6 的过渡条款要求在途分支改号，但 hex 线改号后 `V105` 与 `V20260817_1327` 两个号在 `petgo_stag` 都已 applied，而其兜底前提 `ignore-missing` 从未真正开启，导致 staging 长期起不来。 | CLAUDE.md §纪律5/6；`scripts/ci/check-flyway-versions.sh`；`scripts/ci/flyway-legacy-versions.txt` |
...
| **F6** | 数据 | `pet_profiles` **加列 `pet_type varchar not null`**（`CAT`/`DOG`/`OTHER`，UPPER_SNAKE）；**创建后不可修改**（服务端硬拒 + 前端置灰，避免已完成里程碑数据错乱）。Flyway 加列迁移，版本号用时间戳（E7） | 2-2（建列+创建校验）；2-8（编辑禁改）|
...
| **F8** | UX/品牌 | **UX 品牌/视觉真相 = `petgo_app/lib/core/theme/colors.dart`（薄荷绿 `#7FD1AE`，2026-06-04 全面换肤）+ Claude Design mint 原型**，**非** UX markdown 文档。`planning-artifacts/v1.0.0/UX_DESIGN.md`/`UX_EXPERIENCE.md`（现行）与 `incoming/UX_*.new.md` **均为焦糖色旧版（#C8874A）**，不作品牌权威；**不晋升 incoming `.new` 覆盖现行**（覆盖=品牌回退，否决提案 §2.4 该步）。新功能 UX/交互结构（个人中心重组 / 名片6区块 / 红色态去导航「我已知晓」/ 里程碑三级动效 / 双视图日历 / 推送扩展）以 **PRD §4 + mint 原型**为准，落地一律套 mint token（colors.dart 第①层新 token 优先）。incoming `.new` 仅作新功能交互结构的参考底稿 | 全局 UX；提案 §2.4 修正 |
| **F9** | 数据/口径 | **成长日历「事件日期」与发布时间分离**：`content_posts` 加列 `event_date date null`（仅成长日历类型有值；日常/科普为 null）。**排序口径**：Feed / 「我的发布」按 `created_at` 倒序；成长档案时间线 / 日历视图 / H5 名片快乐时刻流按 `event_date` 排序。事件日期**不可选未来**、可选任意过去；日历格子取「该 `event_date` 下最早 `created_at` 记录的首图」为背景；未来日期格子置灰不可点。Flyway 加列用时间戳号（E7） | 2-3（建列+发布表单事件日期字段+默认值/校验）；2-4（双视图按 event_date + 当天详情页 + 未来格子置灰）；2-6（H5 快乐时刻流按 event_date） |
...
| `places` / `place_photos` / `place_comments` / `place_checkins` / `place_reports` | **V1.3.0 后台 5.1**（`V20260909_1749__init_places.sql`） | 五表 → **本分支（后台）定 schema，App 分支（FR-112）读写数据**（架构 delta 契约 X-1 / X-3）。语义：`status != 'ACTIVE'` 或 `deleted_at IS NOT NULL` 对用户端「不存在」；`MERGED` 行的 `merged_into_id` 供 App 直链跳到保留场所；5.3 合并发 `PlaceMergedEvent(long keepPlaceId, long mergedPlaceId, Instant mergedAt, long actorAdminAccountId)`（`com.tailtopia.admin.places.event`，合并事务内发布；子表照片 / 评论 / 打卡已改指保留场所，举报不迁移），护照章归并由 App 分支用 `@TransactionalEventListener(AFTER_COMMIT)` + `REQUIRES_NEW` 监听实现（同一用户 A、B 都盖过 → 并一枚、次数相加）。`place_type` / `tags` 值域由 App 端定，表不加 CHECK。D-39 加 `city`。详见 `docs/reference/db-schema-reference.md` §8。**2026-09-18 对齐**：batch-b1 另建的一套已删除；App 需要的列由 `V20260918_2110__align_places_for_app.sql` ALTER 追加；计数缓存列删除、后台实时统计；`place_type` 加 CHECK（值域 = App `PlaceType`）、`tags` 值域 = App `PlaceTag`；App 标记的城市经 `PlaceCityResolver` 取默认城市（本版 Jakarta，预留多城市）；后台实体 JPA 名 `AdminPlace*`（规格 `specs/spec-v130-places-schema-alignment.md`） |
| `pricing_config` | 9.2（V78） | **V1.3.0 后台 6.1**（`V20260909_1843__add_pricing_config_passport_prices.sql`）加 `passport_page_unlock_price` / `passport_boarding_unlock_price`（初始 = 当时 `id_hd_download_price`），三价 `CHECK (>= 1)`（D-7）。**契约 X-4**：App 分支 FR-120 护照样式解锁按既有 `GET /api/v1/pet-profiles/me/id-cards/pricing`（`IdCardHdPricingResponse`，⚠️ 实际路径是 `/id-cards/pricing`，架构 delta 写的 `/id-card/hd-pricing` 为笔误）响应新增的 `passportPageUnlockPrice` / `passportBoardingUnlockPrice` 取价，旧字段 `price` 不变；改价只影响新发起的解锁。加样式 = 加列 + 迁移 + 小发版（D-3），不改 key-value |
