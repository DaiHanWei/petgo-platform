...
| **E6** | 基建（**反转 E2 序号分配**，2026-08-21 hanwei 拍板） | **Flyway 迁移号一律时间戳**：新迁移命名 `V<yyyyMMdd_HHmm>__<snake_case>.sql`（取创建时刻；Flyway 把版本段的 `_` 视为 `.`，`20260821.1435` 数值上恒大于存量序号，天然排后），**禁止再用序列号**。背景：三条并行工作线（main V1.1.4 社区管控 / shawn 电商 / hex V1.1.6）在 **V101–V127 三方硬撞**（同号不同内容，同树即 Flyway「more than one migration with version X」启动即崩），序号 + 人肉分号段被证明必然撞车（shawn 曾在分支注释里声明「独占 V101–V139」但声明只存在于他自己分支，main 照样占了 V101–V104/V107/V108 并已应用到 prod）。配套铁律：① `out-of-order=true` 全环境常开（application.yml 已有，时间戳跨分支合并不保证递增，乱序补应用是预期行为）；② **已合入 main 的迁移文件冻结**（不改不删不改名，改表另起新迁移——承 E2 原精神）；③ **CI 守门** `.github/workflows/flyway-guard.yml` + `scripts/ci/check-flyway-versions.sh`（本地可跑 `bash scripts/ci/check-flyway-versions.sh origin/main`）：树内同号 / 动冻结文件 / 新增非时间戳格式 / 新号与 main 已有号相同 → PR 不绿；④ 存量序号迁移（main ≤V108 及各分支返工改号前的历史）**保持原号不动**，在途分支（shawn/hex）的序号迁移合 main 前须改成时间戳号（shawn 的从未应用到任何服务器、改号零风险；hex 的已应用在 petgo_stag，改号需同步处理 stag `flyway_schema_history` 或重建 stag 库） | CLAUDE.md §纪律5；`.github/workflows/flyway-guard.yml`；`scripts/ci/check-flyway-versions.sh`；取代 shawn 分支 `待发-4-Flyway改号方案.md` 的 V140–V164 序号方案 |
