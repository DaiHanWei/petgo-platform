...
| **F18** | 数据生命周期（**前端占位待后端**） | **宠物档案删除端点缺失**：拟新增 `DELETE /api/v1/pet-profiles/me`（走 `/me` 主体，C1；账号注销 7-3 是另一回事）。来源：2026-06-18 UI 保真——`pet-edit` 原型含「🗑 Hapus Profil」危险区，但 `api_paths`/`ProfileRepository` 无删除能力。**当前前端编辑页已落红色删除按钮 + 二次确认弹框，确认后仅占位提示（不真正删除）**。后端待办：建端点 + **级联处置须按 D1 明确**——成长档案派生数据（`health_events`/`pet_milestones`/`milestone_completions`）随档案物理删；档案关联的 UGC（`content_posts` 成长日历条目）保留 vs 删除需产品定 + 名片 `card_token` 失效。**红色态/合规护栏不变**。**需产品先拍板删除语义（物理删 vs 软删冷静期）再实现** | 前端 `pet_profile_edit_page.dart` 占位已落；后端端点 + 删除语义待拍板 |
