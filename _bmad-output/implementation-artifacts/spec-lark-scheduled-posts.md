...
## 2026-08-27 增量：发布账号（邮箱）/ 图片前缀序号 / 备注列

- **表头**（运营已改，9 列）：序号 · 内容分类 · 内容编号 · 文案部分(最多1000字) · 图片编号 · 发布账号(邮箱) · 上传状态 · 发布账号 · 备注。表头匹配取**括号前文字**；「发布账号」有两列——括号含「邮箱」的是**输入**，无括号说明的是**输出**（回写昵称）。缺「备注」列 → 轮级中止。
- **发布账号(邮箱)**：空 → 作者池随机虚拟账号（原逻辑）；非空 → `findByEmailAndRole(email, USER)` 精确匹配且 `status=ACTIVE`（真实号/虚拟号不限——官方运营号是真实注册账号，2026-08-28 拍板），匹配不到 / 格式非法 → 该行**无效**。
