...
**~~🔴 报告里写死的一条硬约束~~：~~Epic 10 未完成时不要跑「全量」验收~~** ——
✅ **2026-09-11 解除**：Epic 10 六条 story 已全部落地（`aafefd12` → `71049c85`）。
报告的〇节前置状态、二节逐页表（泳道 5 的 13 页已展开）、七节未能确认项**均已更新**，
**分母恢复为 8 泳道全部页面**。

同批给报告加了一节 **五之二「泳道 5 专项：L1 本该兜住、这次没兜住的一层」**（14 条 + 3 条「不该有」的）。
原因是泳道 5 有一个别的泳道都没有的特点：

> Epic 10 六条 story 全部在云端执行，而云端**没有 Docker daemon** ——
> 六条新增的 **24 条 MockMvc 用例一条都没跑过**（只验证了编译）。
> 泳道 5 这 13 页是**唯一一批「L0 绿、L1 未验、L2 未验」的页面**，其余泳道至少有 L1 垫底。

那 14 条的失败形态是同一类：**服务端一切正常、日志干净，运营看到的是「点了没反应」**
（抽屉 id 拼不上、oob 没换行、确认框没弹、按钮一直是灰的）。这类缺陷 L0 照不出来、
L1 本该先炸，现在只能靠实机。**实机时先过这一节，再过逐页表。**
