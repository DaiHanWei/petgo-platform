...
- [ ] **T1 · 五个 fragment 壳**（AC1/AC2/AC3）
  - [ ] `tpl-a-workbench.html`：
    ```html
  ...
  - [ ] `tpl-b-list.html`：`filters` 槽 → `.sum` 槽 → `table` 槽 → `<aside id="${res}-drawer" class="drawer" hidden><header>…✕</header><div class="drawer-body"></div></aside><div class="drawer-mask" hidden>`；`res` 由调用方 `th:with` 传入或从 `drawer` 槽的 id 读
  - [ ] `tpl-c-report.html`：`range`（切换器）→ `.cards` 网格 → `detail`；根元素 `data-readonly` 渲染只读标识
  - [ ] `tpl-d-config-card.html`：`<form class="cfg-card" data-config-card>` + 标题 + `form` 槽 + `<button class="btn btn-primary" disabled data-save>`；`admin-core.js` 已有 `data-confirm`；保存钮激活逻辑在本 story 的 `admin-workbench.js`? **不**——放 `admin-core.js`（配置页与工作台无关）：监听 `input/change` 于 `[data-config-card]` 内 → 与初始序列化值比对 → 有差异启用保存钮并打「已修改」标
  - [ ] `tpl-e-steps.html`：`<ol class="step-bar">` 由 `steps` 槽渲染 → `body` → `<footer class="sticky-footer">` `footer` 槽（上一步/下一步）
  - [ ] 每壳顶部注释：槽位契约 + 首个消费者 story 号

- [ ] **T2 · `admin-drawer.js`**（AC4）
  - [ ] IIFE，`window.Admin = window.Admin || {}`；`Admin.drawer.open/close/sync`
  - [ ] 事件：`document.addEventListener('click', tr[data-drawer-url] 委托)`；`keydown Esc`；`.drawer-mask click`；`document.body.addEventListener('admin:drawer-close', …)`
  - [ ] `?open=` 深链：`DOMContentLoaded` 读 `URLSearchParams`，找 `tr[data-id=…]`，无匹配则忽略（该 id 不在当前页）
  - [ ] 焦点管理：打开时记住 `rowEl`，关闭后 `rowEl.focus()`

- [ ] **T3 · `admin-workbench.js`**（AC5）
  - [ ] `htmx:afterSwap` 监听：`e.detail.target.closest('[data-workbench]')` 内且 `e.detail.elt` 根带 `data-next-id` → `selectNext`
  - [ ] 队列行高亮：移除其它 `.q-row.on`，给目标加 `.on`；窄屏 `matchMedia('(max-width:1023px)')` 切 `data-pane`
  - [ ] 提交中态：`htmx:beforeRequest` 给 `e.detail.elt` 最近的 `button` 加 `.is-loading` + `disabled`，`htmx:afterRequest` 移除
  - [ ] 🔴 明确**不**注册键盘 ↑/↓（D-14，全组）；文件头注释写明

- [ ] **T4 · `admin-core.js` 增量**（AC1 模板 D 保存钮）
  - [ ] `[data-config-card]` 未修改禁用逻辑（序列化 `FormData` 比对）；「已修改」标 `<span class="tag-dirty">`
  - [ ] 「有未保存修改时切页/关抽屉拦一次」（9-7 第 6 条）：`beforeunload` 仅在存在 dirty 卡时提示

- [ ] **T5 · CSS**（AC6）
  - [ ] `admin.css` 末尾新段 `/* ===== V1.3.0 模板壳（Story 2.3b） ===== */`；不改 `:root`；对照 UI 稿 `:root` 令牌（照抄自 admin.css，无差异）
  - [ ] 层级（9-4）：`.drawer-mask z:40`、`.drawer z:50`、`dialog z:60`、`.toast z:70`
  - [ ] 行三态（10-1）：默认白 / hover `--brand-tint` 淡 / 选中 `--brand-tint` + 左 3px `--brand`

- [ ] **T6 · kitchen-sink**（AC7）
  - [ ] `admin/web/AdminKitchenSinkController`（`@StagOnly`）+ `templates/admin/_kitchen-sink.html`；五壳假数据；抽屉 fragment 端点 `GET /admin/_kitchen-sink/{id}/drawer`；处置端点 `POST /admin/_kitchen-sink/{id}/done` 返回带 `data-next-id` 的 fragment 演示自动下一条
  - [ ] 不进导航、不进 `AdminPageCatalog`、不进写操作清单（脚本按 `_kitchen-sink` 前缀排除——在 Story 2.1 脚本里加一行排除或在 11.1 diff 时忽略）

- [ ] **T7 · 测试**（AC1/AC8）
  - [ ] `AdminTemplateShellRenderTest`（L0）：五壳各一条，用 `SpringTemplateEngine` + `Context` 渲染，断言槽位 HTML 出现在预期容器内、缺槽位渲染为空不抛异常
  - [ ] `AdminTemplateStructureTest` 若递归扫 `fragments/`，五壳需满足「内容在 fragment 内」规则
  - [ ] kitchen-sink MockMvc：stag profile 200、默认 profile 404
  - [ ] 既有全量回归

- [ ] **T8 · 云端执行须知**
  - [ ] 云端 L0：`mvn -B clean package -DskipITs`（含渲染单测）
  - [ ] L2：本地 stag 起后开 `/admin/_kitchen-sink`，对照 UI 稿泳道 9（9-1～9-6）、泳道 10（10-1～10-8）逐项核
