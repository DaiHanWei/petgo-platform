...
- [x] **T1 · `admin/shared/web/HxRequest`**（AC1）
  - [x] `record HxRequest(boolean htmx, String target, String trigger, String currentUrl)` + `HxRequestArgumentResolver implements HandlerMethodArgumentResolver`，注册进 `WebMvcConfigurer`（查项目现有 `WebMvcConfigurer` 实现，有则追加；无则新建 `admin/shared/web/AdminWebMvcConfig`）
  - [x] `HxRequest.isHtmx()`；静态 `HxRequest.of(HttpServletRequest)` 供过滤器/advice 复用

- [x] **T2 · `AdminBusinessExceptionAdvice`**（AC2/AC3）
  - [x] `@ControllerAdvice(basePackages="com.tailtopia.admin")` + `@Order(Ordered.HIGHEST_PRECEDENCE)`，**只在 `HX-Request` 头存在时接管**，否则 `throw ex` 交回 `GlobalExceptionHandler`（它对 `AppException` 出 ProblemDetail、对 `AccessDeniedException` 重抛给 Security 链）
  - [x] `@ExceptionHandler(AppException.class)` → `ModelAndView("admin/fragments/tpl-shared :: inline-error", status=ex.getStatus() 为 4xx 时保留原状态否则 422)`，model：`message=messages.resolve(ex)`, `code=ex.getMessageCode()`；响应头 `HX-Reswap: innerHTML`、`HX-Retarget`（优先 `HX-Target` 头，其次 `#admin-inline-error`）
  - [x] `@ExceptionHandler(AccessDeniedException.class)` → 403 `tpl-shared :: forbidden`；所缺权限码：从 `AuthorizationDeniedException.getAuthorizationResult()` 拿不到码时，回退解析 `HandlerMethod` 上的 `@PreAuthorize` 表达式里的 `hasAuthority('x')` 字面量（正则），取第一个非 SUPER_ADMIN 的码
  - [x] 🔴 `GlobalExceptionHandler.handleAccessDenied` 现状是 `throw ex` 让 Security 链 forward `/admin/denied`；本 advice 排序更高且只吃 htmx 请求，非 htmx 路径不受影响——写测试钉住
  - [x] 🔴 `BindingResult`/`MethodArgumentNotValidException`（表单校验）在 htmx 下同样走 422 inline-error，文案取第一个 field error

- [x] **T3 · `fragments/tpl-shared.html`**（AC2/AC3）
  - [x] `th:fragment="inline-error(message)"`：`<p class="err" role="alert" th:text="${message}">`
  - [x] `th:fragment="forbidden(permission)"`：`<p class="err muted" th:text="#{admin.v130.err.forbidden(${permission})}">`
  - [x] `th:fragment="toast(message)"`：复用 layout 的 `.toast` 样式，供 oob 追加
  - [x] 三语 key：`admin.v130.err.forbidden=需要「{0}」权限` / en / id

- [x] **T4 · `AdminNavController` + `fragments/badges.html`**（AC4）
  - [x] `GET /admin/nav/badges`：注入 `AdminPageCatalog`（1.5）判可见队列 + `AdminBadgeService.counts(principal)` 返回 `Map<queueKey,int>`；现阶段五队列 count 分别调 `ManualReviewService` / `UnifiedTicketQueryService` / `ConsultAnomalyService` / `AdminSupportTicketQueryService` / `AdminRefundQueryService` 的既有 pending count（先读各类现有方法名，无则加一个 `countPending()`；**禁止**在 Controller 直接注入 Repository，AD 模式规则）
  - [x] fragment：`<span id="nav-badge-total" class="badge" hx-swap-oob="true">23</span>` + 每队列 `<span id="nav-badge-manual-review" hx-swap-oob="true">12</span>`；0 时渲染空串（隐藏）
  - [x] `@Transactional(readOnly=true)` 一次事务；不写日志

- [x] **T5 · `AdminFragmentResponses` / `AdminHxEvents`**（AC5）
  - [x] `AdminHxEvents.BADGE_REFRESH`、`DRAWER_CLOSE`；`AdminFragmentResponses.triggerBadgeRefresh(HttpServletResponse)` 写 `HX-Trigger` JSON（多事件合并成一个对象）

- [x] **T6 · `AdminTime`**（AC6）
  - [x] `@Component("adminTime")`；`fmt/fmtDate/today`；WIB 常量引用 `shared/schedule/ScheduleWindow.WIB`
  - [x] 现状 `AdminContentScheduleController` / `SeedBatchExcelService` 各自定义了 `ZoneId WIB`——本 story 不动它们（Story 7.5 / 7.6 顺手改用 `AdminTime`）

- [x] **T7 · `AdminExportWriter`**（AC7）
  - [x] 参考 `AdminPaymentExportService.exportXlsx` 的 POI 写法（`XSSFWorkbook` / `Sheet` / `Row` / 数字单元格 / `autoSizeColumn`），抽成通用：`xlsx(String sheet, List<String> headers, List<List<Object>> rows)`，`Number` → 数字单元格，其余 `toString`
  - [x] `csv(headers, rows)`：RFC 4180 + 公式注入防护；调用方负责 `'﻿' +` BOM（现状 `AdminContentManageController:194` 已这样做，保持）
  - [x] 单测 `AdminExportWriterTest`（L0）
  - [x] 现有三处导出（支付 xlsx / 内容 csv / 用户召回 xlsx）**本 story 不迁移**，由各页 story 迁（7.1 / 8.1 / 8.5）

- [x] **T8 · 范式接入 + 测试**（AC8）
  - [x] `AdminAccountAdminController.deactivate`：加 `HxRequest hx` 参数；htmx 时返回行 fragment（可先返回 `tpl-shared :: toast` 占位 + `triggerBadgeRefresh`），否则原 PRG
  - [x] MockMvc 四条（见 AC8）；`AdminBusinessExceptionAdviceTest` 单测覆盖 htmx / 非 htmx 两路
  - [x] 全量既有测试回归

- [x] **T9 · 云端执行须知**
  - [x] 云端 L0：`mvn -B clean package -DskipITs`；MockMvc 切片属 L0/L1 边界，云端可跑
  - [x] 无 L2
...
- 云端 L0：`./mvnw -B clean package`（排除需真库的 Spring 上下文测试类）→ 1738 tests, 0 failures, BUILD SUCCESS；复审后定向重跑 `AdminExportWriterTest / AdminFragmentResponsesTest / AdminBusinessExceptionAdviceTest` 10/10。
- 新增 L0：`AdminBusinessExceptionAdviceTest` 6/6（standalone MockMvc + 真 Messages：htmx AppException → 422 + HX-Reswap/HX-Retarget；无 HX-Target 回退 `#admin-inline-error`；非 htmx → ProblemDetail；表单校验 → 422；htmx AccessDenied → 403 forbidden 带权限显示名；非 htmx AccessDenied 原样重抛）、`HxRequestTest` 2/2、`AdminFragmentResponsesTest` 2/2、`AdminTimeTest` 2/2、`AdminExportWriterTest` 2/2（逗号 / 引号 / 换行 / `=SUM` 注入 / 中文 / 纯数字文本放行 / xlsx 数字单元格）。
- `check-i18n-keys.sh` OK（2209 key）；flyway-guard 树内 174 支（本 story 无迁移）。
- 复审：① `HX-Trigger` 头解析对嵌套对象误取 key → 改按花括号深度扫描；② CSV 公式注入防护会给 `"-1"` 这类纯数字文本加 `'` → 放行纯数字文本、前导 Tab 加引号；③ `HandlerMethod` 参数在「异常早于 handler 解析」时无法注入——admin 包内 AccessDenied 只来自方法级 `@PreAuthorize`（handler 必在），保持；④ 停用两分支各调一次服务属可读性取舍，不改。

### Completion Notes List

- **L1/L2 待本地验收**：① `AdminHtmxParadigmIntegrationTest.deactivateFourWays`（真 Thymeleaf：整页 302 / htmx 成功 200 toast + `HX-Trigger` / htmx 停用自己 422 inline-error + `HX-Retarget` / htmx 无权限 403 forbidden）；② 非 htmx 无权限仍 forward `/admin/denied`（Security 链）；③ 浏览器端 422/403 fragment 实际 swap 依赖 2.2 的 `htmx:beforeSwap` 放行，stag 联调时验证。
- **角标端点已在 2.2 落地**（`admin/shared/web/AdminNavController` + `shared/nav/NavBadgeService` + `fragments/nav-badges.html`），本 story 不重复建 `admin/web/AdminNavController` / `fragments/badges.html`；AC4 已满足：一个只读事务、仅可见队列、响应不带 `HX-Trigger`（fragment 只带 `admin:badge-refresh from:body` 触发器，无 `load`，不自触发）。
- 403 fragment 按 **D-37 固定显示所缺权限名**，无开关；权限显示名复用既有 `perm.<code>` 三语 key（story 写 `admin.perm.<code>`，不重复造 key）；拿不到码时 `admin.v130.err.forbiddenGeneric`。
- 非 htmx 路径不是 `throw ex` 而是**显式委托** `GlobalExceptionHandler.handleApp / handleValidation`：Spring 的 `ExceptionHandlerExceptionResolver` 在 advice 方法抛异常后不再尝试其它 advice，直接 `throw` 会让非 htmx 的 AppException 变 500。AccessDenied 非 htmx 仍原样重抛（与现状一致）。
- `AdminTime` 注册为 `@Component("adminTime")`，模板 `${@adminTime.fmt(x)}`；WIB 取 `ScheduleWindow.WIB`；既有 `AdminContentScheduleController` / `SeedBatchExcelService` 的本地 WIB 常量不动（7.5 / 7.6 改）。
- `AdminExportWriter`：现有三处导出不迁（7.1 / 8.1 / 8.5）。
- `HxRequest` 由 `AdminWebMvcConfig`（新 WebMvcConfigurer，与 `AdminLocaleConfig` 并列）注册解析器。
- 待拍板已闭合（D-37）；「各 service 加只读 count 方法」在 2.2 已按仓库 count 方法落地（`NavBadgeService` 经服务 / 仓库，Controller 不直连 Repository）。

### File List

- petgo-backend/src/main/java/com/tailtopia/admin/shared/web/{HxRequest,HxRequestArgumentResolver,AdminWebMvcConfig,AdminBusinessExceptionAdvice,AdminFragmentResponses,AdminHxEvents}.java（新增）
- petgo-backend/src/main/java/com/tailtopia/admin/shared/time/AdminTime.java、shared/export/AdminExportWriter.java（新增）
- petgo-backend/src/main/java/com/tailtopia/admin/account/web/AdminAccountAdminController.java（deactivate 范式接入）
- petgo-backend/src/main/resources/templates/admin/fragments/tpl-shared.html（新增）
- petgo-backend/src/main/resources/i18n/messages{,_zh_CN,_en,_id}.properties（+2 key）
- 测试：admin/shared/web/{HxRequestTest,AdminFragmentResponsesTest,AdminBusinessExceptionAdviceTest}、shared/time/AdminTimeTest、shared/export/AdminExportWriterTest、admin/account/AdminHtmxParadigmIntegrationTest（新增）；AdminAccountAccessControlTest
