...
- [ ] **T1 · 服务层 `rename`**（AC1 / AC2）
  - [ ] `AdminAccountService` 新增 `@Transactional public void rename(long accountId, String displayName, long actorAccountId)`
  - [ ] 校验顺序：findById（`admin.err.account.notFound`）→ trim → 空 → 超长（>100，与列 `display_name VARCHAR(100)` 一致）→ 与旧值相同 return → `a.setDisplayName(v)` → save → 审计
  - [ ] `AdminAccount` 目前**没有 `setDisplayName`**（只有 `setPasswordHash` / `setStatus` / `setRole`），需新增；沿用「setter 最小暴露」风格
  - [ ] `AuditActions` 加 `ACCOUNT_RENAMED = "ACCOUNT_RENAMED"`，放 `ACCOUNT_*` 段
  - [ ] 🔴 审计 summary **不记密码 / 不记 PII 以外的东西**：只记「旧名 → 新名（邮箱）」，与既有 `ACCOUNT_ROLE_CHANGED` 文案风格一致（邮箱在既有审计里已作标识出现，沿用）

- [ ] **T2 · self 护栏**（AC3）
  - [ ] `deactivate`：`findById` 之后、幂等判断之前加 `if (accountId == actorAccountId) throw AppException.validation("不能停用自己的账号").code("admin.err.account.selfDeactivate");`
  - [ ] `changeRole`：`findById` 之后、`newRole == null` 判断之后、幂等判断之前加同型判断 `.code("admin.err.account.selfRoleChange")`
  - [ ] 🔴 放在幂等判断**之前**：对自己「改成同一个角色」也应被拒（语义是「不能对自己操作」，不是「没变化」）

- [ ] **T3 · Controller**（AC1）
  - [ ] `AdminAccountAdminController` 新增 `@PostMapping("/admin/accounts/{id}/rename") @PreAuthorize(CREATE_AUTH)`，参数 `@RequestParam("displayName") String displayName`，try/catch `AppException` → `flash.addFlashAttribute("error", msg.resolve(e))`，成功 `flash notice msg.get("admin.flash.account.renamed", id, displayName)`，`return "redirect:/admin/accounts"`——与既有五个 POST 完全同范式（PRG）
  - [ ] `populate(model)` 增 `model.addAttribute("selfId", admin.getAdminAccountId())`：`accounts(...)` 方法签名加 `@AuthenticationPrincipal AdminUserDetails admin`

- [ ] **T4 · 模板**（AC1 / AC4）
  - [ ] `admin-accounts.html` 操作列 `.account-actions` 内新增「改名」`<details>`（与既有「改权限」`<details class="perm-details">` 同结构）：summary 按钮 `#{admin.accounts.rename}`，内含 `<input name="displayName" th:value="${a.displayName}" maxlength="100" required>` + 保存钮 + 取消（`<button type="button" onclick="this.closest('details').removeAttribute('open')">`），`sec:authorize` 同 `CREATE_AUTH`
  - [ ] 停用表单与改角色表单：`th:with="isSelf=${a.id == selfId}"`，按钮 `th:disabled="${isSelf}" th:title="${isSelf} ? #{admin.accounts.selfGuard} : null"`；改角色的 `<select>` 也 `th:disabled`
  - [ ] 🔴 停用表单带 `data-confirm`：禁用按钮时 `admin.js` 的 data-confirm 不会触发（按钮 disabled 不产生 submit），无需改 JS
  - [ ] 本 story **不**套模板 B（Epic 6 Story 6.5 的事）；只在现有页面样式上加行内操作

- [ ] **T5 · i18n**（AC6）
  - [ ] 四包各加 8 个 key（见 AC6），印尼语参考既有 `admin.accounts.*` / `admin.err.account.*` 措辞
  - [ ] 手工 diff 三包 key 集合（`scripts/ci/check-i18n-keys.sh` 由 Story 2.2 建）

- [ ] **T6 · 测试**（AC5）
  - [ ] L0 `AdminAccountServiceTest`：`renamePersistsTrimmedNameAndAudits`、`renameRejectsBlank`、`renameRejectsOver100`、`renameSameValueIsNoOp`（verify auditService 未调用）、`renameDoesNotBumpSecurityVersion`
  - [ ] L0 `AdminAccountRoleServiceTest` / `AdminAccountServiceTest`：`cannotDeactivateSelf`、`cannotChangeOwnRole`（含「改成同角色也拒」）；`lastSuperAdmin*` 两条既有测试仍过
  - [ ] L0 `AdminAccountAccessControlTest`：`renameNeedsCreateAccountAuthority`（无权 403；`admin.create_account` 200/302；SUPER_ADMIN 302）
  - [ ] L1 `AdminAccountManagementIntegrationTest`：改名后 `list()` 回显新名、审计表新增 `ACCOUNT_RENAMED` 一行、`security_version` 不变
  - [ ] L2（本地）：登录为 A，A 行「停用」「改角色」禁用带 title；B 行可用

- [ ] **T7 · 云端执行须知**
  - [ ] 云端只跑 L0：`mvn -B clean package`（surefire 单测）；L1 集成测试需 Docker postgres + redis，留本地
  - [ ] Completion Notes 标注「L1/L2 待本地验收」
