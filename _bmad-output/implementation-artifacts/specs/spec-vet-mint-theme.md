...
context:
  ...
  - '{project-root}/_bmad-output/_archive/fidelity-rework-2026-06/fidelity-audit.md'
---
...
**Always:** 以原型 H5 (`_bmad-output/_archive/pages/vet-*.html`) 为色值唯一标准；用户侧紫色体系**保持不动**；vet token 走 `AppColors`，不在 vet 业务文件硬编码 hex（遵 CLAUDE.md 护栏）；新增 token 一律 additive，现有常量值/名零改动。
