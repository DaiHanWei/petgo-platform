...
context:
  - '{project-root}/_bmad-output/implementation-artifacts/v1.0.0/2-1-媒体基建-oss-双桶-sts-直传-签名-url-与-exif-剥离.md'
...
- `docs/runbooks/oss-setup.md` -- 单桶 + 预签名配置与验收说明（替 oss-ram-setup.md）
...
- [x] 配置/文档：`.env` 单桶值（去 STS）；`.env.example`/`application.yml` 去 STS 加 uploadTtl；`docs/runbooks/oss-setup.md`；`scripts/local/oss_l2_accept.py`（gitignored）。
