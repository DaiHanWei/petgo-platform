...
**为什么必须事先说：**
- 两人各建一个 `V101__xxx.sql`（文件名不同）→ **git 合并干净，无冲突** → Flyway 启动时报「Found more than one migration with version 101」
- 更隐蔽的一种：你先把 V105 应用到 stag，我的 V101–V104 后到 → Flyway 默认 `outOfOrder=false`，**直接拒绝启动**
...
> **① Flyway 号段**——我占 V101–V139（V101/V102 已提交），你从 V140 起可以吗？两边都从 V101 开始的话 git 不会冲突但 Flyway 会起不来，07-11 已经因此全量重排过一次。
