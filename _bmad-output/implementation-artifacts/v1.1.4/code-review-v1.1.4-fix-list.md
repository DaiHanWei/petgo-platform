...
- **冲突点**：根 CLAUDE.md「对外暴露标识一律不可枚举 token，不用自增 id 直接外露」；`docs/reference/analytics-posthog-tracking.md:79`「不传任何 userProperties」；D4 决议只豁免无盐哈希，并点名合规替代（backend 下发 analytics_token / HMAC）。文件头注释（14-15 行）仍称「只传哈希」，与代码自相矛盾。
