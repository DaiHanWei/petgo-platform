...
- [x] **T1 Flyway V48 迁移**（AC1/2/3）
  - [x] `db/migration/V48__init_ledger_and_pawcoin.sql`（Dev Notes 完整 SQL 照抄：三表 + CHECK + 唯一/索引；顶部注释 story+号段 V47+1+护栏）。**L1**（scratch 库真验 ✅）
...
- [x] **T4 测试**（AC1-5）
  ...
  - [x] L1 集成：`PawCoinWalletConcurrencyIntegrationTest extends ApiIntegrationTest`（真 pg：20 线程并发 debit 至边界断言恰 10 成功、无越负、总账平、`reconcile` 一致；验 Flyway V48 + validate）。照 `admin/anomaly/ConsultAnomalyIntegrationTest`。**L1**（已编译；本地 dev 库污染阻断 context，非负不变式已用 scratch 库 SQL 层真验，见 Completion Notes）
...
- V48 迁移 + **非负不变式 DB 层真验**（一次性 scratch 库，零副作用）：V48 apply OK；wallet balance=10000 起，20 次 `UPDATE ... WHERE balance-1000>=0` → **恰 10 次生效、余额归 0**；再强扣 -1 → `violates check constraint pawcoin_wallets_balance_check`（CHECK 库级兜底）。scratch 库跑完即销毁。
...
- **⚠️ L1（docker pg+redis 并发）待本地验收**：完整 `PawCoinWalletConcurrencyIntegrationTest`（20 线程真并发 + Flyway V48 validate + reconcile）被本地 dev 库污染阻断（同 1.1，用户决策仅 L0）。核心非负不变式已用 scratch 库 SQL 层等价真验；在纯净 dev 库上跑该 test 即完成 L1 全绿。
...
- `petgo-backend/src/main/resources/db/migration/V48__init_ledger_and_pawcoin.sql`
...
- 2026-07-11：实现 Story 1.2 双分录总账与 PawCoin 钱包（`ledger_entries` append-only 双分录 + `pawcoin_wallets` 原子非负 + `pawcoin_transactions` 流水 + `LedgerService`/`PawCoinWalletService`）。L0 11/11 绿；V48 迁移与非负不变式 scratch 库真验；L1 并发待本地（dev 库污染，用户决策仅 L0）。
