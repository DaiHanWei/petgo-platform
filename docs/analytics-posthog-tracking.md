...
### 9.7 🚩 两条非代码交付项（**产品负责，代码这边做不了**）

1. **上线前必须先量基线**：上线前连续 4 周的周均待处理工单数 / 工单构成 / 人均举报次数。
   ⚠️ **上线后才想起来就没有基线可对了，§4.3 整节度量也就白写了。**
2. **回看安排**：上线后**第 4 周，只回看一次**，产出一条明确结论（达成 / 未达成 / **数据不足**），记入决策日志。**无论结果如何都要写，不留悬空。**
