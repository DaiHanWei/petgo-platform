...
**BMAD 技能的版本定位是强制的**（`_bmad-output/project-context.md`「版本解析规则」，所有技能启动时自动加载）：从提示词或分支名解析版本 → 路径重定向到 `<ver>/` 子目录 → 解析不出就问，**绝不回退根目录默认路径**。因此**给云端 session 的提示词必须带版本号**（如「V1.3.0 dev-story 1-1」），或直接在 `feat/1.3.0-*` 分支上跑。
